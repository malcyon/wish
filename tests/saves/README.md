...
| `test_world_state.py` | Checks that `goldbox/world_state.py`'s `WorldState` reads a title's own C64 and DOS specimens to the same answer and carries the five fields added over `PorSaveState`, and that `pod_from_dos` reads a synthetic Pools of Darkness container into `PodWorldState`, and that `amiga_savegame.pod_from_amiga` reads the same party to an equal state, from a synthetic slot and from every played one. |
