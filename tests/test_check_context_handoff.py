...
`.claude/skills/orchestrate/SKILL.md` says to hand off at 600k tokens, but the
...
def test_a_session_past_the_line_is_refused(tmp_path, monkeypatch, capsys):
    path = transcript(tmp_path, _turn(
        "assistant", input_tokens=32, cache_read_input_tokens=600_000))
    assert run(monkeypatch, spawn(path)) == 2
    err = capsys.readouterr().err
    assert "600,032" in err
...
def test_a_session_under_the_line_is_let_through(tmp_path, monkeypatch):
    path = transcript(tmp_path, _turn(
        "assistant", input_tokens=32, cache_read_input_tokens=599_000))
...
def test_the_last_turn_counts_not_the_largest(tmp_path, monkeypatch):
    ...
    path = transcript(
        tmp_path,
        _turn("assistant", cache_read_input_tokens=700_000),
