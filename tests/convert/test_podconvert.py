...
def test_dos_write_refuses_an_icon_head_past_the_art_librarys_thirteen():
    ...
    char = neutral.NeutralCharacter("test", source="made up", game=POD.key)
    char.set("icon_head", 250, "test: past the thirteen heads")
...
def test_write_pod_refuses_an_icon_head_past_the_art_librarys_thirteen():
    ...
    char.set("name", "TEST", "made up")
    char.set("icon_head", 14, "test: one past the thirteen heads")
    with pytest.raises(ValueError):
        amiga_pod.write_pod(char)[0].to_bytes()
...
def test_the_item_records_come_back_past_their_text():
    """The same claim `tests/convert/test_doswriter.py` makes for Pool of Radiance:
