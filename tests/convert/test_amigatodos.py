...
direction reuses whole (`tests/convert/test_amigatoc64.py` owns those tests).
...
def _game_dir() -> pathlib.Path:
    ...
    directory's parent, the way `tests/convert/test_convert.py`'s own `_game_dir`
    finds it."""
...
def shipped_adf(tmp_path) -> pathlib.Path:
    ...
    inside a `.zip`. `tests/convert/test_amigatoc64.py` owns the search and the
