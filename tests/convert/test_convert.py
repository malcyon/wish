...
def game_files():
    ...
    search `tests/convert/test_dosimport.py`'s `files` fixture does."""
...
def test_a_refusal_on_construction_is_shown_not_swallowed(tmp_path,
    ...
    **Adapted, not a straight port.** `tests/convert/test_dosimport.py`'s original
...
def _amiga_por_disk():
    ...
    `tests/convert/test_amigatoc64.py` owns the search and the reason it matches on
