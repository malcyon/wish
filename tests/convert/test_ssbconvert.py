...
The twin of `tests/convert/test_curseconvert.py`, and it exists as a second file
...
def test_no_dos_derived_or_constant_field_reaches_the_import_pane():
    ...
    -- the same guard as `tests/convert/test_dosconvert.py`'s Pool of Radiance
    version and `tests/convert/test_curseconvert.py`'s Curse one.  `icon` and
...
def test_a_silver_blades_conversion_needs_no_creation_tables():
    ...
    `tests/convert/test_dosconvert.py`, where `None` means the stored menu."""
    from editor.dosimport import GameFiles, rehearse
...
def test_a_converted_party_shows_no_portrait_or_identity_drop_line():
    ...
    identity -- and shows neither kind now.  The remaining line was the
    party's one running effect, which turned out to be the paladin's own
    cure timer: it is now written through `write_c64_save`'s own payload and
    slot (#600, #628), so the party shows no drop line at all."""
    from editor.dosimport import GameFiles, rehearse
...
#
# The twin of `tests/convert/test_curseconvert.py`'s section, and it exists because
