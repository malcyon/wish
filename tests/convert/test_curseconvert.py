...
def test_the_flag_window_runs_to_the_end_of_the_page():
    """The twin of `tests/convert/test_ssbconvert.py`'s test of the same name (#289).
...
def test_no_dos_derived_or_constant_field_reaches_the_import_pane():
    ...
    `tests/convert/test_dosconvert.py`'s Pool of Radiance version, on Curse's own
...
def test_a_converted_party_shows_no_portrait_or_identity_drop_line():
    ...
    takes on Silver Blades.  The paladin's cure-disease count and his
    lay-on-hands timer, which used to be the two remaining lines, are now
    written through `write_c64_save`'s own payload and slot (#600, #628)."""
    from editor.dosimport import GameFiles, rehearse
