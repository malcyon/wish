...
def shipped() -> bytes:
    ...
    for _name, data in _amiga_images():
        ...
        if len(save) == amiga_savegame.POR_SAVEGAME_SIZE:
            return save
...
def _c64_state(name: str) -> amiga_por.PorSaveState:
    ...
    return amiga_savegame.por_state_from_c64(payload, str(disk))
...
def test_the_shipped_saved_game_carries_its_own_areas_ecl_block(
    ...
    """
    area = amiga_savegame.por_word(shipped, 0x49F2)
    body = amiga_dax.block(ecl_dax, area, "ecl.dax")[amiga_savegame.POR_ECL_HEADER:]
...
def test_a_c64_party_builds_a_saved_game_that_owes_nothing_to_anybody(
    ...
    save, report = amiga_savegame.new_por_savegame(state, "B", 6, ecl_dax)
    assert len(save) == amiga_savegame.POR_SAVEGAME_SIZE
    assert len(report.sources) == amiga_savegame.POR_SAVEGAME_SIZE
...
def test_a_dos_party_builds_one_too(ecl_dax):
    ...
    state = amiga_savegame.por_state_from_dos(savgam, "SAVGAMD.DAT")
    save, report = amiga_savegame.new_por_savegame(state, "D", 1, ecl_dax)
    assert report.unwritten == []
    # The place is the DOS save's own, read two ways.
    x, y, facing = dos_savegame.position(savgam)
    assert (save[amiga_savegame.POR_POS_X], save[amiga_savegame.POR_POS_Y]) == (x, y)
    assert save[amiga_savegame.POR_POS_FACING] == facing * dos_savegame.FACING_SCALE
...
def test_the_party_stands_where_the_source_save_says(ecl_dax):
    ...
    assert save[amiga_savegame.POR_POS_X] == state.x
    assert save[amiga_savegame.POR_POS_Y] == state.y
    assert save[amiga_savegame.POR_POS_FACING] == state.facing * 2
    for i, digit in enumerate(state.clock):
        assert amiga_savegame.por_word(save, dos_savegame.CLOCK + i) == digit
    assert amiga_savegame.por_word(save, dos_savegame.SCRIPT) == state.area
    assert amiga_savegame.por_word(save, dos_savegame.AREA) == state.geo
    assert save[amiga_savegame.POR_PARTY_SIZE_BYTE] == 6
    assert amiga_savegame.por_word(save, dos_savegame.PARTY_SIZE) == 6
...
def test_the_shipped_saved_game_round_trips_except_where_it_is_declared(
    ...
    """
    state = amiga_savegame.por_state_from_amiga(shipped, "the shipped slot A")
    built, report = amiga_savegame.new_por_savegame(state, "A", 6, ecl_dax,
                                           portraits=True)
    unexplained = []
...
def test_the_regions_a_player_would_notice_round_trip_byte_for_byte(
    ...
    """
    state = amiga_savegame.por_state_from_amiga(shipped, "the shipped slot A")
    built, _report = amiga_savegame.new_por_savegame(state, "A", 6, ecl_dax,
                                            portraits=True)
    start, end = amiga_savegame.POR_ECL_BUFFER
    ...
    assert built[amiga_savegame.POR_VIEW_TYPE:amiga_savegame.POR_PARTY_SIZE_BYTE + 1] == \
        shipped[amiga_savegame.POR_VIEW_TYPE:amiga_savegame.POR_PARTY_SIZE_BYTE + 1]
    for n in range(6):
...
def _outdoor_dos_state() -> amiga_por.PorSaveState:
    ...
    return amiga_savegame.por_state_from_dos(savgam)
...
def test_a_party_on_the_travel_grid_gets_the_bytes_the_engine_writes(ecl_dax):
    ...
    assert save[amiga_savegame.POR_VIEW_TYPE] == amiga_savegame.POR_VIEW_TYPE_OVERLAND == 3
    assert save[amiga_savegame.POR_WALL_BYTE] == amiga_savegame.POR_WALL_OUTDOORS == 14
    assert amiga_savegame.por_word(save, dos_savegame.INDOORS) == 0
    assert amiga_savegame.por_word(save, dos_savegame.AREA) == 0
    assert amiga_savegame.por_word(save, dos_savegame.SCRIPT) == 26
    assert (amiga_savegame.por_word(save, dos_savegame.TRAVEL_X),
            amiga_savegame.por_word(save, dos_savegame.TRAVEL_Y)) == (7, 29)
    assert amiga_savegame.por_word(save, dos_savegame.DISK) == 7
    assert report.unwritten == []
...
def test_an_indoor_party_still_gets_the_indoor_pair(ecl_dax):
    ...
    save, _report = amiga_savegame.new_por_savegame(state, "B", 6, ecl_dax)
    assert save[amiga_savegame.POR_VIEW_TYPE] == amiga_savegame.POR_VIEW_TYPE_3D == 1
    assert save[amiga_savegame.POR_WALL_BYTE] == 0
    assert amiga_savegame.por_word(save, dos_savegame.INDOORS) == 1
    assert amiga_savegame.por_word(save, dos_savegame.TRAVEL_X) == 0
    assert amiga_savegame.por_word(save, dos_savegame.TRAVEL_Y) == 0
...
def test_the_engines_own_outdoor_saved_game_round_trips(ecl_dax):
    ...
    state = amiga_savegame.por_state_from_amiga(savgam, "WISH-SPEC-por-amiga-outdoor B")
    assert state.outdoors is True
    assert state.travel == (7, 29)
    assert state.area == 26
    built, report = amiga_savegame.new_por_savegame(state, "B", 6, ecl_dax)
    unexplained = []
    ...
    assert built[amiga_savegame.POR_POS_X:amiga_savegame.POR_POS_FACING + 1] == \
        savgam[amiga_savegame.POR_POS_X:amiga_savegame.POR_POS_FACING + 1]
    assert built[amiga_savegame.POR_VIEW_TYPE] == savgam[amiga_savegame.POR_VIEW_TYPE]
...
def test_only_as_many_names_are_written_as_the_party_has(ecl_dax):
    ...
    """
    state = _c64_state("por-party-twin-pair")
    save, _report = amiga_savegame.new_por_savegame(state, "C", 2, ecl_dax)
    assert save[amiga_savegame.POR_PARTY_SIZE_BYTE] == 2
