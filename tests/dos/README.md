...
| `test_dosacceptance.py` | Checks that `tools/dos/dosacceptance.py`'s rest-menu keys reach the asked time from any preset on a fake that does the menu's arithmetic, that its camp bar reading ignores the highlight, and that its reading names aged, vanished and expected effect nodes. |
