...
| `test_dosacceptance.py` | Checks that `tools/dos/dosacceptance.py`'s rest-menu keys reach the asked time from any preset in all four titles on a fake that does the menu's arithmetic, that its step orders, stages, Curse party-menu load, save and training and Pools of Darkness' load, sheet, items, save and walk behave on fakes, that its reading names aged, vanished and expected effect nodes and each Pools of Darkness character's thief skills and items, that the fixture and later-title C64 parties convert with a Bless that reads back, and that a synthetic Amiga Pools of Darkness slot converts through the Convert route, installs and reads back. |
