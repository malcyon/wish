...
| `test_dosacceptance.py` | Checks that `tools/dos/dosacceptance.py`'s rest-menu keys reach the asked time from any preset in all three titles on a fake that does the menu's arithmetic, that its step orders, stages, Curse party-menu load, save and training behave on fakes, that its reading names aged, vanished and expected effect nodes, and that the fixture and later-title C64 parties convert with a Bless that reads back. |
