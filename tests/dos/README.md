...
| `test_dosacceptance.py` | Checks that `tools/dos/dosacceptance.py`'s rest-menu keys reach the asked time from any preset in all four titles on a fake that does the menu's arithmetic, that its step orders, stages, Curse party-menu load, save and training and Pools of Darkness' load through `LOAD FROM WHERE?` with `P` pressed once, its journal question answered after `Begin` and before any step with nothing but `X` and `Return` typed into it, its `YES NO` bars declined with `N` alone, in turn and to a bound, its continue dialogs answered with `Return` alone, five at most in a row and twelve screens at most in `begin`, counted one screen at a time, a second `Return` or `N` held back when the first lands late, a town services bar stopping `begin` with a shot and no key pressed, its `begin` taking the dungeon or the overland map bar and stopping at any other screen, its party-menu rows after a save with the training byte set, sheet, items, save and walk behave on fakes, that a sheet in camp or from the party menu's `view` is the roster line asked for, the highlight moved with `Down` and read off the white line, never `End`, even while the camp picture animates, and a sheet whose name or frame belongs to another line stops the run, that every sheet captured in the acceptance runs cached on this machine was roster line 1's, that its reading names aged, vanished and expected effect nodes and each Pools of Darkness character's thief skills and items, that the fixture and later-title C64 parties convert with a Bless that reads back, and that a synthetic Amiga Pools of Darkness slot converts through the Convert route, installs and reads back. |
...
| `test_dospod.py` | Checks the step grammar of `tools/dos/dospod.py`, where it finds the Pools of Darkness game directory, that it reaches the party menu without ever pressing `Return` on it, and on synthetic frames that the journal question is known only by both its digests and gets one letter and `Return`, a second round when the letter is swallowed, and a `TimeoutError` rather than a third. |
