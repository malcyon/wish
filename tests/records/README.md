...
| `test_paladinc64.py` | Checks `goldbox.c64_codec.write` and `read`'s payload/party-slot/clock-minutes arguments: a paladin's cure-disease and lay-on-hands bytes and their rows in the save's shared effect arrays, written for every source and read back, with a disk-backed check against `WISH-SPEC-curse-600-paladin6-cured`. |
