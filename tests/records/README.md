...
| `test_backstab_multiplier.py` | Checks `goldbox/backstab.py` against each title's and port's multiplier table at levels 1 to 20, the level and multiplier caps, the regained former thief level, and the refusal of a title and port with no measured rule, with no game data. |
...
| `test_effects.py` | Checks `goldbox/effects.py`: the effect-array offsets, writing and clearing an effect, the ECL65 spell-effect table, and how much time a duration byte has left at a time of day, and which byte the closest-duration rule picks, against the three engines' own camp ageing routines. |
...
| `test_xpceiling.py` | Checks, from each DOS engine's own `GAME.OVR`, that experience is a 32-bit running total nothing caps below the C64 record's three bytes, and that a record past that width is clamped to `0xFFFFFF` with a drop line by the conversion rather than wrapped or refused. |
