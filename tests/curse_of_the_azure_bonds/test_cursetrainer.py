...
rows that `tests/curse_of_the_azure_bonds/test_curselevels.py` reaches at payload offset `0x88D`.
...
def _c64_specimen(name: str):
    ...
    the same check `tests/curse_of_the_azure_bonds/test_cursedualtrain.py` makes, duplicated rather
...
def trainer_measured_for_this_test(monkeypatch):
    ...
    `tests/curse_of_the_azure_bonds/test_cursedualtrain.py`'s own `measured` fixture does and
...
def test_the_spell_slot_rows_sit_where_ecl65s_own_code_reads_them():
    ...
    `tests/curse_of_the_azure_bonds/test_curselevels.py` reaches the slot rows at payload offset
