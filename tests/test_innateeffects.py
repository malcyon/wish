...
from goldbox import c64_codec, dos, dos_layout, games, neutral
...
def test_both_later_titles_map_the_same_paladin_pair():
    ...
    assert dos.C64_CLASS_TRAITS[CURSE.key] == \
        ((dos.PALADIN_CLASS_BIT, PALADIN_C64_TRAIT, PALADIN_EFFECT),)
    assert dos.C64_CLASS_TRAITS[SSB.key] == \
        ((dos.PALADIN_CLASS_BIT, PALADIN_C64_TRAIT, PALADIN_EFFECT),)
...
def test_the_guarded_bit_is_the_paladins_in_both_titles_own_tables():
    ...
    for game in (games.CURSE_OF_THE_AZURE_BONDS,
        ...
        assert dict(game.class_bits)[dos.PALADIN_CLASS_BIT] == "paladin"
    assert "paladin" not in dict(games.POOL_OF_RADIANCE.class_bits).values()
...
def test_pool_of_radiance_has_no_row_and_translates_nothing():
    ...
    and stays one."""
    assert POOL.key not in dos.C64_CLASS_TRAITS
    assert dos._from_c64_class_traits(
        POOL.key, dos.PALADIN_CLASS_BIT, [PALADIN_C64_TRAIT]) \
        == [PALADIN_C64_TRAIT]
    assert dos._from_c64_class_traits(None, 0xFF, [PALADIN_C64_TRAIT]) \
        == [PALADIN_C64_TRAIT]
...
def test_a_c64_paladins_seed_reaches_the_spc_file_as_the_dos_id(shape):
    ...
    char = _c64_neutral(shape.key, name="TESTER",
                        class_bits=dos.PALADIN_CLASS_BIT,
...
def test_a_cleric_paladin_is_still_a_paladin(shape):
    ...
    for bits in (2 | dos.PALADIN_CLASS_BIT, 8 | dos.PALADIN_CLASS_BIT):
...
def test_a_c64_curse_paladin_converts_with_protection_from_evil():
    ...
    assert paladin.get("class_bits") & dos.PALADIN_CLASS_BIT
    assert PALADIN_C64_TRAIT in paladin.get("innate_effects")
    _rec, _itm, spc, rep = dos.write(paladin)
    assert spc == bytes.fromhex("080000ff0000000000")
    assert _innate_drops(rep) == []

    _rec, _itm, spc, rep = dos.write(by_name["RANGER"])
    ...
    for name in names:
        for char in _c64_party(_c64_disk(name)):
            if PALADIN_C64_TRAIT not in (char.get("innate_effects") or []):
                continue
            assert char.get("class_bits") & dos.PALADIN_CLASS_BIT, char.get("name")
            _rec, _itm, spc, rep = dos.write(char)
...
def test_a_c64_silver_blades_paladin_converts_with_protection_from_evil():
    ...
    """
    root = gamedisks.find(games.SECRET_OF_THE_SILVER_BLADES.key)
    if root is None:
