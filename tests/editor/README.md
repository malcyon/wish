...
| `test_saveasui.py` | Checks the split Open and Save buttons and their arrow menus, the destination section's visibility and fields per platform, every refusal and failure text wired to its exact `editor.saveplan`/`editor.files` exception, the stale-plan silent re-prepare, and the unsaved-changes guard shared with `close()`. |
