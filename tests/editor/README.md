...
| `test_savepublish.py` | Checks that a Save As prepares a native copy or a conversion in memory, refuses one whose accounting names a dropped field or a loss, publishes it at a chosen image filename or into a new save folder with a backup of anything it replaced, and puts the destination back when publication or adoption fails. |
