...
def test_an_unsaved_dos_edit_converts_and_leaves_the_save_folder_alone(
    ...
    not mean two conversions)`: `Source.detect` used to reread a DOS source
    off disk whatever the editor held, so a conversion converted the save as
    it was opened. The player's own folder is byte-identical afterwards --
    a Save As must never save the original first.
    """
...
def test_an_unsaved_c64_edit_is_in_the_payload_a_conversion_reads(tmp_path):
    """A C64 source has carried unsaved edits all along -- but only because
    `EditorBinding.convert` wrote them into the open payload first. The
    snapshot is what carries them now, so `party.save0` and the image are
