...
`tools/dos/dosbox.py` drives the *game* -- keystrokes in, screen digests and save
...
Displays :90-:105 are this pool's; `tools/dos/dosbox.py` has :50-:65 and VICE has
...
widened to sixteen slots: :40-:55 would have landed inside `tools/dos/dosbox.py`'s
...
#: This pool's instances live beside `tools/dos/dosbox.py`'s, one directory down,
#: so the two lease files can never be the same file.
WORK = dosbox.WORK / "x"
INST = WORK / "inst"

#: :50-:65 are `tools/dos/dosbox.py`'s and :10-:25 are the VICE pool's.
DISPLAY_BASE = 90
SLOTS = 16
...
def missing_tools() -> list[str]:
    ...
    `TOOLS` above and not `tools/dos/dosbox.py`'s: `XSession` narrows the list it
...
class Slot(dosbox.Slot):
    """`tools/dos/dosbox.py`'s lease on this pool's own displays and directories."""
    pass
...
def claim(note: str = "") -> Slot:
    ...
    for n in range(SLOTS):
        ...
        if display_num == -1:
            ...
            os.close(fd)
            raise dosbox.PoolFull(
                f"the DOSBox-X display band :{DISPLAY_BASE}-:{DISPLAY_BASE + SLOTS - 1} is full"
            )
...
#: "DOSBox": that would also find `tools/dos/dosbox.py`'s window on a shared display.
TITLE = "wishdbg"
...
def halve(screen: dosbox.Screen) -> dosbox.Screen:
    ...
    **DOSBox-X line-doubles this game's 320x200 mode into a 640x400 window and
    DOSBox 0.74 does not** (#204), so every rectangle `tools/dos/dosbox.py`
    ...
    **Every 2x2 block is checked, not inferred from parity** (#215): each row
    ...
    against a cheaper dimensions-only check. `tools/dos/dosframeaudit.py`'s 320
...
class XSession(dosbox.Session):
    ...
    #: find `tools/dos/dosbox.py`'s window.
    TITLE = TITLE
    ...
    def capture(self) -> dosbox.Screen:
        ...
        Every rectangle in `tools/dos/dosbox.py` -- `BAR`, `STATUS` -- is measured
    ...
    def key(self, *keys: str, gap: float = 0.35) -> None:
        ...
        **XTEST after `windowfocus`, not `xdotool key --window`.**  SDL2 ignores
        ...
        `tools/dos/dosbox.py` uses throughout silently does nothing here -- it
