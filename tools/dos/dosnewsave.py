...
the DOS-side twin of `tools/dos/dosdisk.py` and `tools/c64/savecheck.py`.
...
1. `goldbox.dos_codec.new_dos_save` writes the slot into a **staged copy** of the
   game tree -- `tools.dos.dosbox.Session.stage` makes it, so the archives stay
...
def find_c64_save(name: str | None) -> pathlib.Path:
    ...
    `$POR_DISKS`, then `automap.paths.tool_disks()` -- and are read only.
    """
    if name:
