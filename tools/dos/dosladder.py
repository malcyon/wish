...
`tools/dos/dosparty.py` rolled and one level-up out of the clerics' school.  This
...
is a prompt, and it is photographed and pressed through by name.
    ...
    tools/dos/dosladder.py --party $WISH_SPECIMENS/por-dos/WISH-SPEC-por-party-trained-c2 \\
...
`(7,2)`-facing-west entry `tools/dos/dostrain.py` documents is only for a save made
...
def stage_experience(save_dir: pathlib.Path, letter: str,
    ...
    **A multi-class character is staged from the class it is about to train**,
    ...
    from tools.dos.dostrainprobe import XP_AT, XP_SIZE
