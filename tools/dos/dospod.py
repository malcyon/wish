...
The Pool of Radiance driver in `tools/dos/dosbox.py` does not fit this title
...
is `tools/dos/dosbox.py`'s and is used as it stands.
...
So a differential pair is one boot and one command line::
    ...
    tools/dos/dospod.py --out DIR Escape c '!C' Right Escape c '!D'
