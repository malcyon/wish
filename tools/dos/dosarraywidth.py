...
"""How wide an array inside a DOS character record is, from the engine's loop.
    ...
    tools/dos/dosarraywidth.py spells_memorised
    tools/dos/dosarraywidth.py spells_memorised --title curse-of-the-azure-bonds
    tools/dos/dosarraywidth.py --displacement 0x17 --overlay /path/to/GAME.OVR
    tools/dos/dosarraywidth.py spellbook --sites
...
`tools/dos/dosfieldrefs.py` states apply, and the third is the one that bites: a
...
`tools/dos/dosrecordloops.py` and confirmed the declared widths in every title
...
Reads the player's own archives through `tools/dos/dosbox.find_game` and writes
...
def find_overlay(title: str) -> pathlib.Path:
    ...
    `tools/dos/dosbox.find_game` keys on `START.EXE`, which **Pools of Darkness
...
def _window_start(data: bytes, at: int) -> int:
    ...
    `tools/dos/dosrecordloops.py` uses -- truncated to just past the last
