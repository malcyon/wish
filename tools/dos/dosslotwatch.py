...
every state anybody can create)`.  `tools/dos/dosspellslots.py` reads the slot
...
nonzero value first, and the load is what changes it back.
    ...
    tools/dos/dosslotwatch.py --game SECRET --save SPECIMEN_DIR \\
        --slot D --patch 2:0x140=5 --minutes 40

is `tools/curse_of_the_azure_bonds/doscurse.py`'s console over `tools/dos/dosboxx.py`'s DOSBox-X: it
...
from tools.dos.dosspcexpiry import claim_free, name_key  # noqa: E402
from tools.dos.dosvmwatch import boot_settled, code_at  # noqa: E402
