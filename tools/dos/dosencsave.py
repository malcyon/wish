...
`tools/dos/dosshop.py --encumbrance` writes it *after* `open_loaded` has already
...
Three modes, and the first two need no emulator:
    ...
    tools/dos/dosencsave.py --rungs $WISH_SPECIMENS/por-dos/WISH-SPEC-por-party-ladder-rung*
    tools/dos/dosencsave.py --boots LADDER_DIR/rung* THRESH_DIR/rung*
    tools/dos/dosencsave.py --party $WISH_SPECIMENS/por-dos/WISH-SPEC-por-party-ladder-rung1 \\
        --spoil 999 --menu-save A --camp-save B --view-save D

`--rungs` re-runs `tools/dos/dostrainprobe.install`'s own arithmetic over a chain
...
def rungs(dirs: list[pathlib.Path]) -> int:
    ...
    `tools/dos/dostrainprobe.install` writes `enc - was + gold` when it restages
...
def open_spoiled(party: pathlib.Path, out: pathlib.Path, letter: str,
    ...
    This is `tools/dos/dostrain.open_loaded` with one line moved, and the moved
    line is the whole experiment: `tools/dos/dosshop.py --encumbrance` pokes the
