...
`tools/dos/dualclassdos.py code` disassembles the one site it went looking for.
This is the general form, for the sites you find by other means -- a string
you grepped for, a displacement `tools/dos/dosfieldrefs.py` counted, an address a
...
`GAME.OVR` was otherwise a fresh throwaway script every time.
    ...
    tools/dos/dosdis16.py --game SECRET --file GAME.OVR --at 0x1d94a --window 120
    tools/dos/dosdis16.py --path /some/GAME.OVR --at 0x3bd7b --before 40
    tools/dos/dosdis16.py --game SECRET --strings train
...
`tools/dos/dualclassdos.py` does, and every line printed is the true decode of
...
The player's archives are read only, found the way `tools/dos/dosbox.py` finds
