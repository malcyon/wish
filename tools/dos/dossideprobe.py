...
side-1 name yellow.  `tools/dos/dostailprobe.py` showed the yellow name on the
...
fight: what the engine does to a party member it counts as an enemy.
    ...
    tools/dos/dossideprobe.py                       # party position 1 on side 1
    tools/dos/dossideprobe.py --side-slot 3 --label side-fight-3
...
`tools/dos/dosquickprobe.py` proved that with `0x10F` = 1 the combat command
...
from tools.dos.dosquickprobe import LADDER, walk_to_encounter  # noqa: E402
