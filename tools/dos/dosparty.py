...
`tools/dos/dosgnome.py` proved a single character can be rolled under DOSBox and
...
`tools.dos.dosbox.Session.stage`.
...
so, not because the screenshots look right.
    ...
    tools/dos/dosparty.py --classes --out DIR/classes
    tools/dos/dosparty.py --build DIR/party.json --slot C \\
        --out DIR/build
