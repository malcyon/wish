...
`tools/dosarraywidth.py` tallies every `cmp byte [bp-n], imm` in the 90 bytes
...
settle `spells_castable_cleric`, `attack_forms` and `field_83_87`.
    ...
    tools/dosrecordloops.py <title> <hex-displacement>[,<hex>...]
    tools/dosrecordloops.py <title> --field attack_forms --around 2

Reads the player's own archives through `tools/dosbox.find_game` (via
`tools/dosarraywidth.find_overlay`) and writes nothing.
"""
