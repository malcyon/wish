...
scripts inside the guest:
    ...
    tools/winwishsetup.ps1     install Python and PyQt6, unpack the repository
    tools/winwishtask.ps1      run things in the guest's session 1
    tools/winwishmeasure.py    the minimum-width measurement
    tools/winwishrun.py        the visible window

Typical run, from the repository root:

    tools/winwish.py setup            # takes the lease, installs, unpacks
    tools/winwish.py measure          # the numbers, on the Windows style
    tools/winwish.py start            # leaves the window up on the guest
    virt-viewer --connect qemu:///system win11    # a person looks at it
    tools/winwish.py stop
    winvm release <tag>
...
def setup() -> int:
    ...
    for local, remote in ((archive, "C:/Wish/wish-tree.tar.gz"),
                          (ROOT / "tools/winwishsetup.ps1", "C:/Wish/winwishsetup.ps1"),
                          (ROOT / "tools/winwishtask.ps1", "C:/Wish/winwishtask.ps1"),
                          (ROOT / "tools/winwishmeasure.py", "C:/Wish/winwishmeasure.py"),
                          (ROOT / "tools/winwishrun.py", "C:/Wish/winwishrun.py")):
