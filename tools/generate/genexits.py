...
so a party that fast-travelled out kept her. `tools/areas/exitreentry.py` proved
...
`tools/areas/eclexitkinds.py`'s per-exit analysis into the one fact `FastTravel`
...
#: Which of `tools/areas/eclexitkinds.py`'s kinds carry a concrete departure
...
def build(title: str = TITLE):
    ...
    rows, skipped, _combat = build_with_combat(title)
    return rows, skipped


def build_with_combat(title: str = TITLE):
    """`build`'s two results and a third: the `(from_area, to_area)` pairs
    ...
    **A pair counts as combat if any of its routes does**, although only the
    ...
    return rows, skipped, combat


def render(rows, combat=frozenset()) -> str:
    lines = [
        "#: Generated by `tools/generate/genexits.py` from the game's own scripts and "
        "maps.",
    ...
    for (frm, to), (entry, square) in sorted(rows.items()):
        fight = ", combat=True" if (frm, to) in combat else ""
        lines.append(
            f"    ({frm}, {to}): ExitRoute({entry}, {square!r}{fight}),")
    lines.append("})")
    return "\n".join(lines)


def render_report(rows, skipped) -> str:
    lines = [f"# `tools/generate/genexits.py`, {len(rows)} direct routes", ""]
...
def main(argv=None) -> int:
    ...
    rows, skipped, combat = build_with_combat()
    print(render(rows, combat))
