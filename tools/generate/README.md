...
| `genexits.py` | **Generates** `automap/fasttravel.py`'s `EXIT_ROUTES` table from `tools/areas/eclexitkinds.py`'s per-exit analysis: one square for each area pair a scripted exit reaches, and which of `DUNGEON`'s two dispatch points re-enters it there. `--report PATH.md` also writes the exits it could not place and why. |
