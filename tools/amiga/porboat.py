...
holding zero in both.
    ...
    tools/amiga/porboat.py por1.adf --out por1-harbour.adf
    tools/amiga/porboat.py por1.adf --report
...
def report(save: bytes) -> list[str]:
    ...
    x, y, facing = (save[amiga_savegame.POR_POS_X], save[amiga_savegame.POR_POS_Y],
                    save[amiga_savegame.POR_POS_FACING])
    return [
        f"square           ({x},{y}) facing {facing} (doubled)",
        f"wall in front    {save[amiga_savegame.POR_WALL_BYTE]}",
        f"square property  {save[amiga_savegame.POR_SQUARE_PROPERTY]}",
        f"view type        {save[amiga_savegame.POR_VIEW_TYPE]}",
        f"$4AA7            {amiga_savegame.por_word(save, HARBOUR_OFFERS)}",
        f"$4A01            {amiga_savegame.por_word(save, HARBOUR_SPOKEN)}",
        f"$4AC4            {amiga_savegame.por_word(save, PASSAGE)}",
        f"$5200            {amiga_savegame.por_word(save, SQUARE_PROPERTY_WORD)}",
    ]
...
def stage(save: bytes,
    ...
    if len(save) != amiga_savegame.POR_SAVEGAME_SIZE:
        raise SystemExit(f"an Amiga Pool of Radiance saved game is "
                         f"{amiga_savegame.POR_SAVEGAME_SIZE} bytes, got {len(save)}")
    out = bytearray(save)
    out[amiga_savegame.POR_POS_X], out[amiga_savegame.POR_POS_Y] = square
    out[amiga_savegame.POR_POS_FACING] = facing * 2
    out[amiga_savegame.POR_WALL_BYTE] = 0
    out[amiga_savegame.POR_SQUARE_PROPERTY] = 0
    amiga_savegame.por_put_word(out, SQUARE_PROPERTY_WORD, 0)
    amiga_savegame.por_put_word(out, HARBOUR_OFFERS, HARBOUR_OFFERS_SET)
    return bytes(out)
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    path = amiga_savegame.por_save_path(
        amiga_savegame.por_savegame_filename(args.slot.upper()), drawer)
