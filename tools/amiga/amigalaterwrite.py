...
would close that issue -- a converted party in front of the running game.
    ...
    tools/amiga/amigalaterwrite.py --source copy-of-ssb.d64
    tools/amiga/amigalaterwrite.py --source copy-of-ssb.d64 --report
    tools/amiga/amigalaterwrite.py --source copy-of-ssb.d64 \\
...
* `--into ... --out`, which rebuilds a saved game on a copy of an Amiga disk
  ...
  the same call `tools/amiga/amigalaterslot.py` makes when it edits a party that
  was already there.
...
#: they use -- `tools/amiga/amigalaterslot.py`'s constants, not a second guess.
SAVE_DRAWER = "SAVE"
SUFFIXES = (".dat", ".sav")
