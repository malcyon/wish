...
on one machine?)`.
    ...
    .venv/bin/python tools/amiga/fsuaeprobedrive.py [PORT] [--display :77] \\
        [--shots DIR]
...
`tools/amiga/fsuaegdb.py launch` starts one, and `tools/amiga/fsuaerununpackaged.sh` and
`tools/amiga/fsuaeruncustombuild.sh` are the two shell versions. Uses the `Gdb` class
of `tools/amiga/fsuaegdbprobe.py`. Needs `import` (ImageMagick) and `xdotool`.
"""
