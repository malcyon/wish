...
`tools/amiga/amigarecordrefs.py` is the same question asked of a **character
...
Nothing is written, and both disk images are opened read-only.
    ...
    tools/amiga/amiganodefields.py --adf <ssb-disk-1.adf> --exe /Secret \\
        pool --global 7618
    tools/amiga/amiganodefields.py --adf <ssb-disk-1.adf> --exe /Secret \\
        fields --chain 96 --size 10
    tools/amiga/amiganodefields.py --adf <curse-disk-1.adf> --exe /Curse \\
...
from tools.amiga.amiga68k import Executable, load  # noqa: E402
from tools.amiga.amigaglobal import code_range, operand  # noqa: E402
...
def sites(exe: Executable, pattern: re.Pattern) -> list:
    ...
    Decoding each candidate from its own start is what `tools/amiga/amigaglobal.py`
