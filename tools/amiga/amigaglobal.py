...
`tools/amiga/amiga68k.py refs` searches for **PC-relative** references, which is the
...
(`#28 (Decode an Amiga saved game, not just a character file)`).
    ...
    tools/amiga/amigaglobal.py --adf copy-of-secret-A.adf --exe /Secret \
        refs 57a0 57a1 57a2
    tools/amiga/amigaglobal.py --adf copy-of-secret-A.adf --exe /Secret \
        callers 118cc
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not exe.small_data:
        raise SystemExit(
            ...
            "has no d16(a4) globals; use tools/amiga/amiga68k.py refs")
