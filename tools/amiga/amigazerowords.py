...
7. **`/program` off Amiga disk 1**, under `--engine`: every site that loads
   ...
   nothing in the game touches at a statically known address.
    ...
    tools/amiga/amigazerowords.py --dos PATH [PATH...]  add DOS containers
    tools/amiga/amigazerowords.py --engine             add the engine's own accesses
    tools/amiga/amigazerowords.py --json words.json
