...
`tools/amiga/porslot.py` copies an Amiga slot into another Amiga slot, so the only
...
Amiga disk the game will load.
    ...
    tools/amiga/toamigapor.py por1.adf --to B --out por1-B.adf \\
        --c64 ~/wish-specimens/por-c64/WISH-SPEC-por-party-twin-pair.d64
    tools/amiga/toamigapor.py por1-B.adf --to D --out por1-BD.adf \\
...
is what a player is actually handed (#36):
    ...
    tools/amiga/toamigapor.py por2.adf --to B --save-disk poolsave.adf \\
        --c64 ~/wish-specimens/por-c64/WISH-SPEC-por-party-twin-pair.d64
...
def read_c64_party(path: str) -> list:
    ...
    characters, _icons = dos_codec.c64_party(sg0.to_bytes(), save1, game)
    return characters
