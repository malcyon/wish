...
`tools/livelevel.py` is what raises a level once it is known none does, and
`tools/c64savespells.py` is the per-character detail for named disks.

The folder is `--disks`, else the one `tools/gamedisks.py` finds for
`pool-of-radiance` (`$POR_DISKS` wins). Reads only.

    .venv/bin/python -m tools.c64savescan
"""
