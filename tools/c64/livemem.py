...
"""Dump a range of a **running** pooled session's memory to a file.
    ...
    tools/c64/livemem.py --port 6523 0000 10000 ALL.BIN
    tools/c64/livemem.py --port 6523 4b00 1d00 -            # hex to stdout

`tools/c64/porcmd peek` answers one short range as hex, which is right for a
...
process, and `tools/c64/session.py` opens and closes its own for each command, so
