...
the routes `tools/c64/c64u.py` refuses.
    ...
    tools/c64/c64urest.py fileinfo /Temp/temp0002       size and name, or the error
    tools/c64/c64urest.py mount X.D64 [--name X.D64]      POST: upload and mount
    tools/c64/c64urest.py remount /Temp/X.D64           PUT: mount what is there
    tools/c64/c64urest.py run MARKER.PRG             POST: DMA-load and start it
    tools/c64/c64urest.py runfirst POOL1.D64         the image's first PRG
    tools/c64/c64urest.py grab /Temp/X.D64 BACK.D64         FTP RETR
    tools/c64/c64urest.py ftpcheck                      does ftplib reach it at all
    tools/c64/c64urest.py dir BACK.D64                 a local image's directory
    tools/c64/c64urest.py screen                        what is on the C64's screen
    tools/c64/c64urest.py key Y                         one key into the KERNAL buffer
    tools/c64/c64urest.py reset                         PUT /v1/machine:reset
    tools/c64/c64urest.py markprg MARK.PRG --file MARK1
                       build a PRG that writes one sequential file to drive 8
    tools/c64/c64urest.py tempsweep --count 12 --dir DIR
