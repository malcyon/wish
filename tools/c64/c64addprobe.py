...
disagree for the matching question; `tools/c64/c64nametable.py show` prints both
...
def answer(sess, word: str, timeout: float = 60.0) -> bool:
    ...
    (`tools/curse_of_the_azure_bonds/cursewarp.py`).
    """
...
def settle(sess, budget: float, quiet: int = 3) -> None:
    ...
    routine that draws it (`tools/c64/c64nametable.py`'s `wait_still`).
    """
