...
* the stop state anybody can check -- the frozen jiffy at `$00A0`, `$DD00`,
  ...
  against a KERNAL ROM with `capstone`'s MOS65xx backend (`--kernal PATH`).
    ...
    tools/c64/c64uhang.py build --out hang.d64
    tools/c64/c64uhang.py run --size 8192 --interval 2 --minutes 10 --log r.jsonl
...
from tools.c64.c64uload import (  # noqa: E402
    Device,
    Log,
    endcheck,
    sample,
...
from tools.c64.c64urest import find_host  # noqa: E402
...
def boot_reproducer(dev: Device, log: Log, image: bytes, answer: str) -> bool:
    from tools.c64.c64urest import first_prg
...
def poll_until_hang(dev: Device, log: Log, size: int, at: int, interval: float,
                    ...
                    blip: float) -> dict:
    from tools.c64.c64uload import blip as blip_wait
