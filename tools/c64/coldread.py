...
which does not move.
    ...
    tools/c64/coldread.py traits secret-of-the-silver-blades
    tools/c64/coldread.py levels curse-of-the-azure-bonds
    tools/c64/coldread.py effects secret-of-the-silver-blades
    tools/c64/coldread.py table secret-of-the-silver-blades 0x17D0 8
...
def disks(game: c64_port.C64Container, root: str | None) -> list[D64]:
    ...
    `root` comes from `--disks`; failing that this asks `automap/gamedisks.py`'s
