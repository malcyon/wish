...
converted to DOS is told so with no way to check it)` asks.
    ...
    tools/c64/rosterspellcount.py                     every C64 save it can find
    tools/c64/rosterspellcount.py --disks DIR         one directory of D64s
    tools/c64/rosterspellcount.py --quiet             just the totals
