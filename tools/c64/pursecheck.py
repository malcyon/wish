...
Two subcommands:
    ...
    tools/c64/pursecheck.py run --save curse-edited.D64 \\
...
taken there instead -- `tools/c64/inventorycheck.py` has that route working on
...
def sheet_up(s) -> bool:
    ...
    `tools/c64/inventorycheck.py:sheet_up` and `CurseSession.sheet_is_up`.
    """
    row = s.row(24)
...
def leave_sheet(sess, r: Run, walk) -> bool:
    ...
    **The sheet's `EXIT` is on the command bar and the picker's is a row in
    ...
    the second character never read.  `tools/c64/inventorycheck.py:press_bar` is
    ...
    """
    from tools.c64.inventorycheck import press_bar
