...
its own READY routine wrote?)`. `tools/c64/traitquery.py` answers the question from
...
taken in the running machine.
    ...
    tools/c64/traitdrive.py --save PORSAVE13.D64 --stage 0:9=21
    tools/c64/traitdrive.py --save PORSAVE13.D64            # the control
...
`tools/c64/session.py` machinery while the count builds. A stopping breakpoint
...
into the pool slot's own directory first, exactly as `tools/c64/statusdrive.py`
does it.
"""
