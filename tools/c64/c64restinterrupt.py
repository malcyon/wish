...
rest there can be interrupted at all.
    ...
    tools/c64/c64restinterrupt.py code                the static reading
    tools/c64/c64restinterrupt.py drive --rests 12    the measurement
...
def overlay_bytes(name: str, root: str, base: int = 0x0800) -> tuple[int, bytes]:
    ...
    from tools.c64 import overlay as O
...
def reach_world(sess, log: Log, seconds: float = 600.0) -> bool:
    ...
    answer (`tools/c64/savecheck.py`'s `answer_bars`).
    """
