...
"""Set a watchpoint on a **running** pooled session and say where it fired.
    ...
    tools/c64/livewatch.py --port 6523 --load 3583 3585

`tools/c64/porcmd` has no way to stop the machine and look at it: a session
...
process; `tools/c64/session.py` opens and closes its own for each command, so this
