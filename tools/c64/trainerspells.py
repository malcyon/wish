...
of spell ids a character of each level comes away with.
    ...
    tools/c64/trainerspells.py --rows
    tools/c64/trainerspells.py --check          # exits non-zero on a mismatch
...
Nothing here needs an emulator. `tools/c64/trainerscan.py` supplies the overlay
and the record base; `tools/c64/d6502.py` is the disassembler behind the
...
from tools.c64.d6502 import M_IMM, SZ, T  # noqa: E402
