...
point it outside the repository.
    ...
    tools/c64/c64u.py info                     device identity
    tools/c64/c64u.py disks                    which images we would mount
    tools/c64/c64u.py boot                     stage disk 1, mount it, reset
    tools/c64/c64u.py read 4900 --length 32    hex dump of one range
    tools/c64/c64u.py dump --start 0 --to ffff --banking default
    tools/c64/c64u.py poll -o DIR             the automapper's own read, once
    tools/c64/c64u.py time-reads --count 100   how many polls a second this gives
    tools/c64/c64u.py probe-key SPACE          does the game read the KERNAL buffer
...
def boot_disk(game: c64_port.C64Container | None = None, number: int = 1,
    ...
    `tools/c64/session.py` stages `POOL1.D64` as `SIDE1.D64` and boots that; sorted
