...
"""Drive a fight with every routing decision logged, and read the log back.
    ...
    tools/c64/combatdiag.py run --out DIR/run1.jsonl --slot 4
    tools/c64/combatdiag.py read DIR/run1.jsonl
...
**Why this is not `tools/c64/stepcheck.py`.** `stepcheck` runs the same routing
