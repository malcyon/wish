...
hits, and without it the payload offset is printed instead.
    ...
    tools/c64/recordsweep.py --game pool --offset 0xB9
    tools/c64/recordsweep.py --game curse --offset 0xB9 --offset 0xBA --context

**A hit is a claim about bytes, not proof they are code** -- the caution
`tools/c64/trainerscan.py` and `tools/c64/d6502.py` both carry. A lone hit inside a
