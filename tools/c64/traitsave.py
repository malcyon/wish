...
`tools/c64/traitdrive.py`, which pokes a `.d64` directly; a byte poked into a save
...
-- the modal wait for a person, and nothing else.
    ...
    tools/c64/traitsave.py write --who ROLAND --trait "Resist Fire"
    tools/c64/traitsave.py boot --disk edited.d64 --who ROLAND
...
is `tools/c64/traitask.py --save <the disk this wrote> --cast ...`, which needs no
...
def rest_hours(sess, log: Log, hours: int, before: dict) -> dict:
    ...
    exactly as `tools/c64/c64restinterrupt.py` does it, and no key is sent while
    the loop runs: `$1E44` reads the keyboard on every tick and a key there
    ends the rest early.
    """
    from tools.c64.c64restinterrupt import CLOCK, REST_MINS, leave_camp, open_camp
