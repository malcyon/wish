...
check, after the jiffy has already been seen to stop.
    ...
    tools/c64/c64uload.py trial --treatment silent --minutes 40 --log B1.jsonl
    tools/c64/c64uload.py trial --treatment readmem --size 7168 --at 4900 \\
        --interval 5 --minutes 60 --log E1.jsonl --capture E1
    tools/c64/c64uload.py endcheck
    tools/c64/c64uload.py fit --log E1.jsonl
    tools/c64/c64uload.py stress --treatment version --interval 0.1 --minutes 20 \\
        --log HTTP1.jsonl
...
#: The regions `tools/c64/c64uplay.py regions` takes, plus the loaded-files cache.
REGIONS = [
