...
`tools/c64/savecheck.py` could not.  Its walk loop is `Session.walk_one`, which
...
must not enter the repository.
    ...
    tools/c64/hallmenu.py --disk SAVE.d64 \
        --out DIR --walk MKI
...
#: address `tools/c64/savecheck.py` reads, and the one the engine restores from
#: `$49F2` on load.
AREA_AT = 0x6E1B
