...
Our judgement rather than the game's data, so it is committed.
    ...
    tools/gui/monsterlabels.py                       # the report, to the terminal
    tools/gui/monsterlabels.py --propose             # add a draft row for every name the YAML lacks
    tools/gui/monsterlabels.py --markdown monster-labels.md
...
# document. `tools/gui/monsterlabels.py --propose` appends a draft row for any
...
def propose(monsters: dict[str, Monster], path: pathlib.Path = TABLE_PATH
    ...
    lines.append(f"\n  # Drafted {stamp} by tools/gui/monsterlabels.py --propose; "
...
def markdown(monsters: dict[str, Monster], labels: dict[str, str],
    ...
    lines = [
        ...
        "`tools/gui/monsterlabels.yaml`** and run "
        f"`tools/gui/monsterlabels.py --markdown {out}` to redraw this; the "
