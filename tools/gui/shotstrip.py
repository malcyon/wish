...
`tools/gui/shotwindow.py` draws the whole window, and it draws it with no
...
grouping or the row's placement wants exactly this picture again.
    ...
    .venv/bin/python tools/gui/shotstrip.py                     # every badge lit
    .venv/bin/python tools/gui/shotstrip.py --effects 1 49      # Bless and Prayer
    .venv/bin/python tools/gui/shotstrip.py --effects none      # nothing running
    .venv/bin/python tools/gui/shotstrip.py --monsters 2        # and on monsters
...
lets every badge be looked at at once. `tools/gui/livestrip.py` is the other half
...
def _offscreen() -> None:
    ...
    a sandbox on its own. `tools/gui/shotwindow.py` has the long version.
    """
...
def shoot(app, party, monsters: int, zoom: int, full: bool, snap=None):
    ...
    `tools/gui/livestrip.py` passes, so the same picture can be taken of a real
