...
|---|---|---|
| `tools/curse_of_the_azure_bonds/cursesavediff.py` `REGIONS`, `tools/secret_of_the_silver_blades/ssbsavediff.py` | "the area map the engine builds on load" | `ANIMATE00`'s picture buffer: the camp scene's current animation frame at the moment of the save |
...
| `tools/curse_of_the_azure_bonds/cursedisk.py`, `tools/secret_of_the_silver_blades/ssbdisk.py` docstrings, `tools/README.md` rows | "the explored map", "the area map" | the picture buffer |
