...
| which classes a race may take | `GEN` `$0E64`, seventeen bytes indexed by the class code, each a bitmask of the races that may take it (bit `race - 1`); DOS keeps the same matrix as seven counted lists of class codes at `START.EXE 0x00F44F`. `tools/records/classlegality.py` reads both | **CONFIRMED for all seven races, on both ports** — the two tables agree on 37 entries over 7 races, and the DOS lists match the creation menus driven in the game for `docs/90-specimens.md` in content and in order. **Only the half-elf may be a cleric/magic-user**, which is what `#510 (Can a Pool of Radiance character memorise more than the 21 spells its DOS record allots?)` turned on |
| ability-score minimums per class | `GEN` `$0E75`, six bytes per class code in the record's own ability order (STR, INT, WIS, DEX, CON, CHA at `0x014`-`0x019`), read by the menu builder at `GEN $0A53` before a class is offered | **CONFIRMED** — the cleric wants wisdom 9, the magic-user intelligence 9 and dexterity 6, the cleric/magic-user both; the paladin's row is the rulebook's 12/9/13/-/9/17 for a class Pool of Radiance never offers. DOS carries a table of the same kind near `START.EXE 0x00F597` whose alignment did not settle — only 3 of 17 rows match the C64's at the best base — so that address is **not** established |
