...
`goldbox.amiga_por.make_por_save_disk` formats it with `AmigaDisk.blank` and puts
...
`' BC       '` is the array `goldbox.amiga_por.slot_list_bytes` writes -- `B` in
...
`goldbox.amiga_por.por_area_script` refuses that area by name, so an area 30 source
