...
(now-lost) `reports/quest-flags.md` §7 established that **the ECL bytecode, absolute
...
any save this project holds is anything but well** -- 38 `.d64` images in
scratch (since deleted) and on the player's own disks carried a `SAVEDGAME1`, and across their 304
...
tables in `DUNGEON` (the write-up, `reports/ecl-opcodes.md`, is lost), with the same mnemonics and
...
| `$1F` | **unimplemented.** Our table called it `ADDRESSOF`, a name inherited from the `coab` table. No Pool of Radiance script uses it — our own sweep counts 0 references — so the name was a guess and is withdrawn (P57); the opcode table, `reports/ecl-opcodes.md`, is lost, but it left `$1F` unnamed before it went |
...
(now-lost) `reports/quest-flags.md` derived the same region from the C64 bytecode
...
`reports/effects.md` decoded structurally without ever getting names for
