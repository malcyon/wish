...
|---|---|---|---|
| **256 effect names** indexed by effect id | `Enum` → `EffectName` | our named effect codes — `reports/effects.md` (lost) named about 30, and a fuller table now lives in `goldbox/traits.py` — against theirs, about 130, and the twenty ids we both hold agree exactly (1 blessed, 2 cursed, 4 manual, 8/9 protection from evil/good, 10 resist cold, 11 charmed, 12 enlarged, 13 reduced, 14 friends, 16 read magic, 17 shield, 19 find traps, 20 resist fire, 21 silenced, 22 slow poison, 23 spiritual hammer). **The effect id space is shared between the ports.** | PROBABLE, and each name promotable one at a time |
