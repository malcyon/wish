...
So the byte carries nothing a player could lose, `goldbox.amiga.write_later`
...
`goldbox/amiga.py`'s `amiga_por_effect_to_dos` maps the other nine bytes onto
...
under WinUAE on 2026-09-07, with `goldbox.amiga.write_later`'s zero at offset
...
Recommended, and **not done here** — `goldbox/amiga.py` belongs to another
agent:
