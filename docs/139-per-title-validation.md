...
in the running game on each, except Silver Blades' badges since they were
added.** What is left is the combat view and log, which
...
| A13 | inventory edit, add, remove | V | V | V | **Closed on 2026-09-08 on both**, VICE pool slot 7, `tools/c64/inventorycheck.py`. All three verbs on one character, made through the editor's own handlers, then read off the game's own `EQUIPPED ITEM` screen with the unedited specimen run through the same code as the control. Curse, `WISH-SPEC-curse-party-with-items`: ten rows became seven, `SILVER MIRROR` went from seven copies to three with a `9` in front of one, `TWO-HANDED SWORD` appeared, and the readied `YES 1 FLASK OF OIL` stayed put. Silver Blades, `WISH-SPEC-ssb-d-engine-resave`: twelve rows became ten, `30 ARROW +1` became `9 ARROW +1`, `CANARY` appeared, and `LONG SWORD +1`, `SHIELD +2` and `PLATE MAIL +1` were gone. `test_inventorycheck.py`, nine tests, including that every byte an edit moves is inside that character's own item page and that a save with no edit writes the payload back byte for byte. **The row's old blocker was out of date twice over**: `#32 (One Curse session, to get a party with items)` made the Curse specimen on 2026-09-04, and `WISH-SPEC-ssb-d-engine-resave` has always carried twelve items on Guy de Valois |
...
| C10 | combat view | V | **U**, and no longer for want of the addresses | **U**, same | `automap/combat.py` holds `$6E11`, `$0600`, `$A380` — PoR's combat overlay — and is still not threaded per title, so the view stays shut on the later two. **The reason has changed and the old one is gone**: all six addresses are known now (`$7F11`, `$0600` unchanged, `$6F00`, `$CB00`, `$6700`, `$92E8`), derived from each title's own binary and read off a running Curse on 2026-09-08 — `docs/101-combat-view.md`, "The same fight in Curse and Silver Blades", `#334 (The session driver cannot fight in Curse or Silver Blades, and says the party is not in a fight while it is standing on the combat floor)`. `tools/c64/latercombat.py` is the table and `tools/c64/session.py` reads a fight through it; what is left for this row is threading the same table into `automap/combat.py` and opening the view. Curse ships no `SQRPACI`/`SQRDATA` at all (`docs/120` tier 1.1) and does not need to: its `COM.PREP` writes the parameter block as immediate constants |
...
`tools/gui/livecheck.py` is how it is re-taken. Two of Curse's and both of Silver
...
**A13 was the sixth until earlier the same day**, when `tools/c64/inventorycheck.py` staged a
...
with `tools/c64/inventorycheck.py` and `tools/c64/pursecheck.py` on the same day.
...
came back to this section. `tools/c64/inventorycheck.py` is the item half and
`tools/c64/pursecheck.py` the money half; neither needed a new session.
...
had no badge groups then and has a row now (C13); the quickfight bit was staged and cleared on all three (C14,
