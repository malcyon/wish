...
Python 3.12 and the `wish` virtual environment, `git`, `gh`, VICE (from Flathub, as `tools/c64/porlaunch.sh` runs it), DOSBox-X (built from source, for its heavy debugger), FS-UAE, and the agent CLIs: Claude Code and Codex, run under `herdr`, the agent multiplexer, with its hooks installed for both. `herdr` and `codex` are not pinned: for each, a script in the guest asks GitHub for the newest release, checks the SHA-256 GitHub publishes for it, installs it under a directory of its own only if that tag is not already there, and moves a symlink. The playbook runs it, so a fresh deployment gets the latest, and a systemd user timer runs it weekly, so the guest keeps itself current. `claude` updates itself. Each agent CLI is logged in by hand once per rebuild.
...
Nothing of the operator's runs inside the guest, so its code is looked at from outside it: the guest's `wish` checkout is mounted on the desktop, read-only, at `~/agent-wish` with sshfs, made by a systemd user unit as the operator, so there is no `allow_other`. It mounts at login with nothing typed, reconnects when the guest reboots, and answers an error rather than hanging when the guest is down or killed: measured, `ls` fails in 3 seconds with the guest off, and after a kill it returns a listing up to 2 seconds old and then fails in 3. The kernel refuses every write.
...
**The App's private key is the only secret in the guest**, with its `config.json`, two integers that are not secret. The role copies both from `agent_guest_wish_agent_src` (default `~/.config/wish-agent` on the desktop) to the same path under the `agent` user, mode `0600` in a `0700` directory, so they never enter this repository. Revoking the key on github.com ends everything the guest can do.
...
| Looking at the code | sshfs, read-only, of the guest's `~/src/wish` at `agent_vm_wish_mount` on the desktop, by a systemd user unit that mounts at login and reconnects; no VS Code Remote SSH |
