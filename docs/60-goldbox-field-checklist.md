...
| Spells **memorized** | Found at `0x020`, a list of ids packed **forward** in descending spell id where DOS fills its own region in reverse. Each port's width is that engine's own loop bound: the C64's `CAMP` counts down from `#$50` over `$6B20`, so **81 bytes, `0x020`–`0x070`** (`tools/memorisedwidth.py`), and DOS's `GAME.OVR` compares a byte counter against `0x14` at four sites indexing `es:[di+0x17]`, so **21 bytes, `0x017`–`0x02B`** (`#508 (A converted magic-user loses memorised spells on the way to DOS, because our table says a title has fewer slots than the engine gives it)`). The high bit of an entry is a flag both engines set and clear, not part of the id. The most anybody has memorised is 13. | 81 B on the C64, 21 B on DOS | — | **CONFIRMED** on both ports, from each engine's own code |
