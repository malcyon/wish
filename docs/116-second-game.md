...
| §7 "`0x100` status byte … rests on one hobbyist editor" | It reads 1 in every occupied record, like Pool of Radiance's `roster_in_use` — and **the editor's enum is now supported by the game itself**: Pool of Radiance's `LIBRARY $38BE` masks the byte with 7 and indexes its own `OK, GONE, DEAD, DYING, UNCONSIOUS, RUNNING, STONED`. Whether Curse's `LIBRARY` does the same has not been checked |
