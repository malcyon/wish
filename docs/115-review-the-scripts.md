...
at that time."* Issues #5 (Read the thirty decoded ECL scripts) and #137 (The ECL decode and the decoder that regenerates it are both gone) are closed with it.
...
`ecl-scripts/`, thirty files, one per `ECL` per disk, regenerated by
`analysis6/ecl6.py`. Both were in gitignored scratch, now deleted, and
neither is recoverable (#136 (Thirty-two cited write-ups are gone, because the knowledge base pointed into gitignored scratch)). **Rebuilding the decoder is the first step of
...
* **Anything that reads or writes a byte we have not named.** 374 distinct
  ...
  established this, `reports/ecl-opcodes.md`, is lost.
...
| | |
|---|---|
| the scripts | `ecl-scripts/dis_POOLn__ECLnn.txt` (scratch, deleted) — not currently present |
| the decoder | `analysis6/ecl6.py` — not currently present |
| the opcode table and cross-reference | lost — was `reports/ecl-opcodes.md` |
| the quest flags | lost — was `reports/quest-flags.md`; the counts are restated above |
| encounters | lost — was `reports/encounters.md`; the finding survives in `docs/50-experiments.md`, "Does the game scale random encounters to the party?" |

All were in scratch (deleted 2026-09-18), which was gitignored — these are derived from the game's own
...
`tools/`; that it did not is the lesson of #136 (Thirty-two cited write-ups are gone, because the knowledge base pointed into gitignored scratch).
...
* **`$1F` is unimplemented.** Our table called it `ADDRESSOF`, a name inherited
  ...
  opcode table itself, `reports/ecl-opcodes.md`, is lost, but it left
