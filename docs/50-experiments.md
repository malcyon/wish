...
`tools/generate/genitems.py` and the document is generated, so the fix went there --
...
`tools/areas/loadfiles.py` prints them for any script; it was `wallart/loadfiles.py` (scratch, deleted)
until `#159 (Nobody has read what Fast Travel skips in the other twenty-nine
scripts)` moved it somewhere it can be found. `tools/areas/eclwalk.py` reads the same
...
`tools/areas/eclwalk.py exits` reaches 79 `NEWECL` statements across the thirty area
...
staying a hypothetical. `automap.actions.reenter()` places the party on the
exit square, rebuilds the stack from `$03BF` (`SP := [$03BF]`, then push the
return addresses a step would have left), and sets the PC to one of
`DUNGEON`'s own post-step points — `$0957` after a landed step, `$0A4C` then
`$0978` for the forward key off a map edge. `NEWECL`'s own tail (`$2034`) does
the same rebuild-from-`$03BF` before its own `JMP $0809`, so the engine does
not trust the stack across an area transition either; the re-entry is doing
the same thing the engine's own transition does, at a different moment, not
something the engine never does. Driven live three times out of three on
`npc_party.d64` with the hand-rolled version of this, `tools/areas/exitreentry.py`
(2026-09-04/05), and once more through the shipped `automap.actions.
FastTravel().run()` itself, unmodified, through a real `automap.target.
ViceTarget` (2026-09-08, driver `tools/areas/fasttravelrun.py`): production code
dropped Princess Fatima from the roster the same way walking out of the
Kobold Caves does. **The shipped path has one live sample, by its author.**
`automap/fasttravel.py`'s `EXIT_ROUTES`, generated by `tools/generate/genexits.py`,
answers "which handler" for the thirteen ambiguous pairs above by keying the
exit on the party's square and facing rather than on the area pair — the
game itself resolves the rest once the party is standing there. `#207 (Run an
exit's own handler before Fast Travel warps out)` has the full history.
...
`tools/generate/genui.py --check` reported every generated file current. The six-case
