...
Neither file is a package member (there is no `__init__.py`, and PyInstaller reads `wish_main.py` as a file path), so `tests/test_packaging.py` and `tests/test_geniconset.py` load their target by file path, because `import packaging.x` resolves to the unrelated PyPI package of that name.
