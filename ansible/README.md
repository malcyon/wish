...
| Tools | `herdr` and `codex`, **not pinned**, each through the same script (`herdr-update`, `codex-update`): it asks GitHub for the newest release, checks the SHA-256 GitHub publishes for it, installs it under `~/.local/share/<name>/<tag>` only if that tag is not there, moves the `~/.local/bin/<name>` link, and keeps the newest two. The playbook runs both and a systemd user timer runs each weekly; use those rather than `herdr update`. `claude` updates itself. `suite-timing` runs the suite and reports wall time and guest memory |
...
| Live tests | `emulator-tests.timer` runs the suite's `livetests/` directory weekly, logs to `~/.cache/wish/emulator-tests`, and files an issue with `tools/wishagent.py` on a failure unless one is already open |
| Node | 20, through `nvm`, on `PATH` for every shell through the same profile drop-in as `WISH_SPECIMENS`; its one use is the VICE MCP server, `~/src/mcp-vice-emu`, pinned at `agent_guest_mcp_vice_commit` with a local patch committed on top (`roles/agent-vm-guest/files/mcp-vice-emu-attach.patch`) and built so `dist/index.js` exists |
