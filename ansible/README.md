...
| Tools | `herdr` and `codex`, **not pinned**, and `c64u`, **pinned** (below). `herdr-update` installs the newest checksum-verified GitHub release under `~/.local/share/herdr`; `codex-update` runs OpenAI's standalone installer so app-server and `/agents` have the managed package tree under `~/.codex/packages/standalone`. The playbook runs both and systemd user timers run them weekly. `c64u`, the C64 Ultimate command-line tool, is installed by the same script at exactly the tag `agent_guest_release_tools` gives it (`v0.9.4`), checked against the SHA-256 GitHub publishes for the asset, with no update timer, and is found by `tools/c64/c64u.py` at `~/.local/bin/c64u`. `claude` updates itself. `suite-timing` runs the suite and reports wall time and guest memory |
