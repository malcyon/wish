...
A section can be run alone by its tag: `boot`, `packages`, `gh`, `claude`, `vice`, `dosbox`, `wish`, `node`, `agenthud`, `herdr`, `codex`, `herdr_integrations`, `environment`, `credential`, `disks`, `exporter`, `login`, `codewheel`, `emulator_tests`; or `agent_vm` for play 1, `agent_vm_guest` for play 2, `isolation` for the test, `agent_network` for the `sandbox-network` role in `sandbox-network.yml`, `winvm` for the `windows-vm` role.
