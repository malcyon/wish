...
|---|---|
| `__init__.py` | Empty. |
| `amiga.py` | Amiga *Pools of Darkness* `Save/NAME.pc` character files, big-endian. Every offset was read off the screen by writing probe payloads onto a copy of disk 3; the record holds base values only, because the game rederives THAC0, encumbrance and movement on load. |
...
| `c64_codec.py` | The writing half of the neutral pair: a `NeutralCharacter` becomes 580 C64 bytes, and every byte is justified as sourced, computed by a named rule, or a documented constant. |
...
| `dos.py` | Reads a DOS Pool of Radiance save and converts it to C64 — one direction only. A DOS field with no C64 counterpart is *reported* rather than dropped silently, which is what `Report` is for. |
| `dos_layout.py` | The 285-byte DOS character record as the same declarative `Field` / `Confidence` table `layout.py` uses, measured against 24 real specimens. Read-only; nothing writes a DOS file. |
...
| `games.py` | Which Gold Box title a save came from, as a table of numbers rather than a class hierarchy. Six C64 titles share one engine and one 580-byte record; what differs is the file names, the load addresses and whether the roster is a second file. |
...
| `neutral.py` | The port-neutral character record every conversion passes through, where each value carries its provenance and not just a number. Three codecs around one record instead of six point-to-point converters. |
...
| `yaml_io.py` | Exports a save's party to YAML and imports it back, losslessly: only understood fields are editable, and the header, the roster tail and the majority of each record still unidentified are carried through byte for byte. |
