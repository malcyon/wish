...
| `dos_savegame.py` | The DOS `SAVGAM?.DAT` saved game mapped (#59): the VM word array indexed by ECL address, the clock, the party size, the wallset triple, the dead ECL buffer, and the retarget recipe — read-only accessors over any save's bytes. |
