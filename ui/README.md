...
icons the program draws — all from game-icons.net now, with one Font Awesome
Free path parked, unreferenced, for a revert. Neither `editor/` nor `automap/`
...
| `icons.py` | The small vector icons the program draws, as SVG path data with no Qt in the file, plus the parser that reads it and `ARTISTS`, which records who drew each game-icons.net glyph so an attribution file can be generated from what actually ships. Two canvases — game-icons.net's glyphs on 512, and `hat-wizard` (Font Awesome, unreferenced, kept for a revert) on 640 — kept verbatim rather than rescaled into one box, so a committed path can be diffed against the artist's own file; `box(name)` is what makes them come out the same size beside each other. Paths rather than a bundled font because the map draws with `QPainter` and not `QIcon`, because a path scales into whatever box it is given, because `render.py`'s `to_svg` then gets the notes for free, and because it costs 21 KB of source rather than a 405 KB binary the packaging has to know about. |
