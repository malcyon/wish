...
icons the program draws — all from game-icons.net now, and nothing from Font
Awesome at all. Neither `editor/` nor `automap/`
owns them — icon drawing is pure geometry and belongs to both, so it lives
here rather than forcing the editor to import from a package named after the
emulator.
...
| `icons.py` | The small vector icons the program draws, as SVG path data with no Qt in the file, plus the parser that reads it and `ARTISTS`, which records who drew each game-icons.net glyph so an attribution file can be generated from what actually ships. Two canvases — game-icons.net's glyphs on 512, and Font Awesome's 640, which nothing is drawn in now — kept verbatim rather than rescaled into one box, so a committed path can be diffed against the artist's own file; `box(name)` is what makes them come out the same size beside each other. Paths rather than a bundled font because the map draws with `QPainter` and not `QIcon`, because a path scales into whatever box it is given, because `render.py`'s `to_svg` then gets the notes for free, and because it costs 72 KB of source rather than a 405 KB binary the packaging has to know about. Thirty-eight game-icons.net glyphs are in it, sixteen of them the map-note kinds added on `#166 (Draw the twenty-three note kinds Donald chose, and name them)`. |
