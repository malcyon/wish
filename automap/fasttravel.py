...
#: its `--report` writes the whole table wherever you point it -- if the
#: reading this rests on, `tools/eclexitkinds.py`'s, ever changes.  No path
#: under `work/` is named here on purpose: `work/` is gitignored, so CI has
#: none, and a citation of a file only this machine has fails there while
#: passing every local run.
#:
