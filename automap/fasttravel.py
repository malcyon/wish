...
#: `tools/generate/genexits.py` from the game's own scripts and maps; run it again --
#: its `--report` writes the whole table wherever you point it -- if the
#: reading this rests on, `tools/areas/eclexitkinds.py`'s, ever changes.  No scratch
#: file is named here on purpose: scratch lives in a temp directory and is
