...
| `convert.py` | The registry step 1 of `#52 (File ▸ Import and File ▸ Export for every direction the library supports)` built: which save conversions `goldbox` can write whole, owing nothing to a template. No window yet — `Source.detect` reads a `.D64` or a DOS save folder off a path, `destinations_for` answers only the directions in `DIRECTIONS` (today: DOS ↔ C64, both Pool of Radiance), and `fresh_folder` names the `wish-YYYY-MM-DD` folder a conversion writes into. `exports.py`'s `Source` retires into this one at step 5. |
| `dosimport.py` | **Off unless `WISH_EXPERIMENTAL_DOS_IMPORT=1`** — the menu entry is not built otherwise, because this direction still drops the portrait and the clock. The window over `goldbox/dos.py`: turning a DOS save into a C64 one with the losses named first. **No template** (`#118 (Write a C64 save from nothing, so importing a DOS save needs no existing .d64)`) — the `.d64` is built from nothing, so the window asks for a DOS folder, a slot and where to put the result, and the import refuses outright when the player's game disks cannot be found rather than inventing the bytes only they can supply. The conversion is rehearsed in memory and what it cannot carry is on screen before there is a button to press; the bottom row names the `.d64` Convert will write, so Convert converts and there is no Save As after it. |
