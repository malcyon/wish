...
| `character.ui` | The Qt Designer form for the character sheet — the actual layout of every widget. The roster and the combat icon sit above three tabs, Stats, Inventory and Spells, and each tab scrolls inside itself because the Character box is 672px of spin boxes and cannot shrink. Edit it in Designer (`designer/`), not by hand; `tools/genui.py` compiles it to `ui_character.py`. |
...
| `window.py` | The main window — roster and combat icon across the top, the sheet on three tabs below. Finds widgets by `objectName` and matches them to `por/layout.py` fields, so the form can be rearranged, regrouped or relabelled in Qt Designer without a line of this file changing. |
