...
| `character.ui` | The Qt Designer form for the character sheet — the actual layout of every widget. The roster, the combat icon and Character sit above three tabs, Stats, Inventory and Spells, and each tab scrolls inside itself because a box of spin boxes cannot shrink. Character is split in two: the five fields that fit beside the icon without putting a font-sized floor under the window, and the other eighteen in Miscellaneous on Stats, which sits to the right of Money so that Abilities and Saving throws are on the left of the window. Edit it in Designer (`designer/`), not by hand; `tools/genui.py` compiles it to `ui_character.py`. |
...
| `window.py` | The main window — roster, combat icon and Character across the top, the sheet on three tabs below. Finds widgets by `objectName` and matches them to `por/layout.py` fields, so the form can be rearranged, regrouped or relabelled in Qt Designer without a line of this file changing. |
