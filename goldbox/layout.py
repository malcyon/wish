...
_DECLARED: Sequence[Field] = (
    ...
    _field(0x012, 1, _U8, "paladin_cures", "Cure disease uses left", _OK,
           "Curse and Silver Blades only; 0 for every other class. GEN seeds "
    ...
    _field(0x020, 69, _RAW, "spells_memorised", "Spells memorized", _OK,
           ...
           "ranges; see goldbox/spells.py. **The roster block's +0x03 onwards "
           "-- record 0x103-0x10B -- are a per-level count of this list, one "
           "byte per spell level, and the earlier retraction of that reading "
           "is withdrawn.** COM.PREP $15ED clears nine counters and rebuilds "
           "them from this field for every live combatant at the start of a "
           "fight: `LDX #$08 / LDA #$00 / STA $6C03,X / DEX / BPL`, then "
           "`LDY #$50 / LDX $6B20,Y / BEQ + / JSR $1751 / TAX / INC $6C02,X`. "
           "$1751 gives a spell id its level with thresholds $16, $24 and $38 "
           "-- goldbox/spells.py's own _GROUPS_POOL boundaries, arrived at "
           "from the spell names alone. Nothing else recomputes them and CAMP "
           "touches none of them, which is what made PORSAVE4 read 0/0/0 "
           "beside a set list: it was saved after memorising and resting, and "
           "before the next fight. Watched happening: +0x03 went 0 -> 1 for "
           "two casters between the world screen and the first round of one "
           "fight, ROLAND's stayed at 3, and the three non-casters stayed at "
           "0 (tools/c64/rosterspellcount.py, #365 (Three roster bytes have no "
           "established meaning, and a C64 party converted to DOS is told so "
           "with no way to check it)). The most seen in use is 13 bytes, and "
           "0x02D-0x070 is zero in every specimen we hold.\n"
           "**69 bytes is what every measured title shares, not the whole "
           "list.** This field used to be declared as sixteen and the note "
           "used to say the length was unproven and PROBABLE at 21, on the "
           "argument that the DOS record allots 21 slots. Both were wrong and "
           "are deleted: the width is not a guess and it is not 21. Each "
           "title's own CAMP walks the list with a count-down loop whose "
           "immediate is the last index, so the width reads straight out of "
           "the code -- 'LDX #$50 / LDA $6B20,X' at CAMP $1450 in Pool of "
           "Radiance, '#$44 / LDA $7C20,X' at $15E2 in Curse of the Azure "
           "Bonds, '#$49 / CMP $7C1B,X' at $1412 in Secret of the Silver "
           "Blades, with twelve to sixteen sites in each and one immediate "
           "across all of them. So Pool of Radiance has **81 slots at "
           "0x020-0x070**, Curse **69 at 0x020-0x064**, and Silver Blades "
           "**74 at 0x01B-0x064** -- a different start as well as a different "
           "width, because Silver Blades' GEN copies seven ability bytes back "
           "to 0x014 where Curse's copies twelve, leaving 0x01B-0x01F free "
           "for spells. All three CONFIRMED; tools/c64/memorisedwidth.py reads "
           ...
           "eighty-one)\n"
           "**Position within the list carries no meaning.** The "
           "engine scans all of it, ignores zeros wherever they fall, draws "
           ...
           "game on the C64: one spell swept across all sixteen positions "
           "of the region as this field was then declared, for "
           ...
           "#110 (Memorised spells lose their slot positions on the way "
           "through the neutral record)"),
    ...
    _field(0x07F, 9, _RAW, "spells_known_high", "Spellbook (high ids)", _OK,
           ...
           "field, and because `set_raw` is width-exact: widening one "
           "declared field would have changed what every writer emitted "
           "in one step, with nothing to catch a caller that had not been "
           "looked at. The writers have since been taken across "
           "deliberately -- `c64_codec`, `yaml_io` and `levelup` all "
           "encode the open title's width over both fields "
           "(#85 (A YAML export of a Curse or Silver Blades party stops at "
           ...
           "0x078-0x084. GEN $232A writes there too, ORing $E0 into $7C81 and "
           ...
           "-- GEN $09DC clears sixteen bytes (LDX #$0F / LDA #$00 / "
           "STA $7C78,X / DEX / BPL), GEN $18C9 walks the same sixteen, and "
           "CAMP $2871 is Curse's memorise loop with its ceiling moved to "
           "CPY #$76, id 117, reaching 0x086.\n"
           "**Every address in the paragraph above was corrected on "
           "2026-09-02 (#31 (Cold-read Curse and Silver Blades for the "
           "fields the editor shows))**, having been written at each "
           "overlay's PRG header base -- $1000, $1220, $3000, $4000 -- "
           "where GEN and CAMP both run at $0800 in all three titles. The "
           "instructions and the widths were right and the addresses named "
           "bytes in the middle of something else, so nobody checking one "
           "with tools/c64/overlay.py, "
           "whose --base is $0800, would have found it. The base is settled "
           "by the operands: Silver Blades' GEN $18C9 reads its own scratch "
           "at $1BFD and its ceiling table at $17D0, and Curse's CAMP $2A25 "
           "calls $1CE9 and stores to $2A1F -- every one of them inside the "
           "file at $0800 and outside it at the header's address.\n"
           "**The near-miss, written down so it is not walked into twice**: "
           "Curse's GEN $220F copies 32 bytes out of $7C78 and looks like "
    ...
    _field(0x073, 1, _U8, "char_class", "Class", _OK,
           ...
           "8 = cleric/fighter, 9 = cleric/fighter/magic-user, 10 = "
           "cleric/ranger, 11 = cleric/magic-user, 12 = cleric/thief, "
           "13 = fighter/magic-user, "
           "14 = fighter/thief, 15 = fighter/magic-user/thief, 16 = "
           "magic-user/thief, 17 = monster. That enumeration is the game's "
           "own: DOS `START.EXE` keeps the eighteen names as counted strings "
           "in code order from 0x00CF06, and they agree with all four "
           "multi-class codes we had already read off the bitmask at 0x0EB. "
           "The 1989 BASIC editor on poolce.d64 displays the same table with "
           "two faults, and this note carried the second of them until "
           "2026-09-15: it lists 3, 4 and 5 all as MAGIC-USER, which is its "
           "author's gap rather than the game's, and it lists 10 as a second "
           "cleric/magic-user. 10 is the cleric/ranger, which Pool of "
           "Radiance implements no further than the name -- `GEN $0E64` "
           "gives the code a race mask of zero, so the creation menu never "
           "offers it. "
    ...
    _field(0x0B8, 1, _U8, "flags_0b8", "Flags", _OK,
           ...
           "is Pool of Radiance's own trainer flag: it records that an "
           "ability score **or the hit points** were changed and kept in "
           "MODIFY CHARACTER. **Only Pool of Radiance writes it** "
           "(docs/232-the-c64-control-byte-per-title.md); Curse, Silver "
           "Blades, Gateway, Champions and Death Knights never touch bit 0 "
           "at all -- their modify screens edit the ability and hit-point "
           "fields directly and neither reads nor writes 0x0B8 for it. "
           ...
           "NPC money is zeroed by it. On Pool of Radiance, bit 0 is set by "
           "GEN $155D straight after INC/DEC $6B14,X, and independently by "
           "$1621 after INC/DEC of hp_max, and cleared again if the change "
           "is cancelled. **Nothing anywhere reads bit 0 back**, so the "
           "forum rumour that altering a score carries a penalty in play "
           "has no code behind it on this port. "
    ...
    _field(0x0B9, 1, _U8, "dual_class_slot", "Dual class: old class slot", _OK,
           ...
           "AD&D 1st edition's rule read off the machine: LDA race / CMP #$07 "
           "(human), a check that refuses on carry clear, LDA 0x0BA / BNE -- "
           "so a character dual-classes at most once -- and LDA level / CMP "
           "#$02, level 2 or better. Only then does $23C9 store the slot and "
           "$23D2 the level. The three consumers are the ones docs/135 names: "
           "$1470 leaves the old slot out of the experience clamp, $1321 out "
           "of eligibility, $15E7 out of the hit-die roll, and $20A3 writes "
           "the old level back into that slot and ORs its class bit into "
           "class_bits. Secret of the Silver Blades (GEN $1FB7/$1FBD) and "
           "Gateway to the Savage Frontier (GEN $23D3) carry the same stores. "
           "**Pool of Radiance does not use these two bytes at all**: no "
           "absolute-mode instruction in any of the 589 files on its disks "
    ...
    _field(0x0BA, 1, _U8, "dual_class_level", "Dual class: old level", _OK,
           ...
           "0x0B9 is a real slot number only when this byte is non-zero. That "
    ...
    _field(0x0E1, 1, _U8, "armour_class_base", "AC base (60 - AC)", _MAYBE,
           ...
           "two"),
    _field(0x0E3, 1, _U8, "strength_bonus_flag", "Strength bonus gate", _OK,
           "gates the to-hit and damage bonus a high strength gives: GEN "
           "$0B79 writes 1 here at creation (Curse's GEN $0C31 the same at "
           "0x7CE3), and LIBRARY's roster recompute at $375C reads it, "
           "indexing strength_index into the AD&D strength tables at "
           "$3651/$3670 only when this is non-zero -- 18 gives +1/+2, 21 "
           "gives +2/+4, 23 gives +3/+6. Curse of the Azure Bonds makes "
           "the same gate at LIBRARY $394B on $7CE3, and its two tables at "
           ...
           "on VIEW, so a record with this zero looks right on the sheet "
           "until the party's first fight. `goldbox.c64_codec.write` left "
           "it zero until #277 (A DOS character converted to the C64 loses "
           "the strength bonus to hit and damage, because 0x0E3 is written "
           "zero) and now writes 1, which is what the C64's own character "
           "creation writes. Named here by #258 (The C64 side of 0x0AB is "
           "unnamed, so the conversion drops it with no issue behind it)"),
    _field(0x0E4, 2, _RAW, "region_0e4", "unknown @0x0E4", _NOPE,
           "between the strength gate and the identity pair. Referenced by "
           "nothing in 589 files and $00 $00 in every player character; "
           "$FF $FF in the five shipped MON* NPC records, which used to be "
           "read as two bytes of an eight-byte NPC marker together with "
           "0x0B9, 0x0BA and others. **There is no marker**, only the "
           "author's $FF fill of the monster files intersected with what a "
           "player character happens to hold zero at -- see "
           "strength_bonus_flag and dual_class_slot, two of the eight that "
           "turned out to be real fields once read rather than assumed. "
           "#224 (0x0B9 and 0x0BA are documented both as an NPC marker and "
           "as the dual-class slot), #258 (The C64 side of 0x0AB is "
           "unnamed, so the conversion drops it with no issue behind it). "
           "Not a candidate region: unlike the identity pair it once sat "
           "beside, it shows no structure of its own -- zero in every "
           "player character we hold, BRUTUS included"),
    _field(0x0E6, 2, _RAW, "identity_pair", "Identity pair", _OK,
           ...
           "with the unreferenced $6BE4-$6BE5 beside them, while the "
           "experience control moved alone. The add screen's own "
           "duplicate test (GEN $1897) compares a candidate's name "
           "against every occupied slot's name and nothing else -- in the "
           "running game a different character under a party member's "
           "name was starred and refused, and the same character under a "
           "new name was let in carrying the identical pair, unnoticed. "
           "This is the C64 home of DOS 0x0AB, the byte the DOS engine "
           "uses for the same tie-break, and it is Pool of Radiance's "
           "alone: Curse of the Azure Bonds' and Secret of the Silver "
           "Blades' GEN never write it, and both hold 00 00 in every "
    ...
    _field(0x0AD, 10, _RAW, "item_effects", "Character Traits", _MAYBE,
           ...
           "**The seeding is per title, and so is the namespace "
           "(#31 (Cold-read Curse and Silver Blades for the fields the "
           "editor shows)).** The "
           "read is the same three instructions in all three measured titles "
           "-- LDX <staging>+0x072 / LDA <table>,X / STA <staging>+0x0AD -- "
           "and what differs is how many slots it fills and what it fills "
           "them with. Curse GEN $24EA seeds three from $24FF, $2506 and "
           "$250D; Silver Blades GEN $0C4B seeds two from $0C5B and $0C62, "
           "and both clear all ten first where Pool of Radiance does not. "
           "**Curse's codes are Pool of Radiance's** -- dwarf 26, 47, 97, "
           "gnome 18, 48, 97, elf 107, half-elf 124, every one landing on the "
           "race its Pool of Radiance name demands. **Silver Blades' are "
           "not**: its elf is seeded 95 and its half-elf 18, which read as "
           "'fights on from -6 to 0 hit points' and a gnome's bonus against "
           "kobolds. Four ids agree (26, 47, the gnome's 48 and the "
           "paladin's 45) and five "
           "do not (95, 18, 7, 92 and the ranger's 105, where Curse GEN "
           "$2515 writes 134), so it is a reassignment and not an offset. "
           "The class seed is written after the racial one, by "
           "Silver Blades GEN $0FF0. #186 (The character sheet gives a "
           "Silver Blades elf a Pool of Radiance ability) gave Silver "
           "Blades its own table "
           "in goldbox/traits.py: the six codes an approved string already "
           "covered are named -- 18 and 95 borrowing the wording of Pool of "
           "Radiance's 124 and 107 -- and 7, 92 and 105 show their number, "
           "because nobody has established what they mean. "
           "tools/c64/coldread.py traits reads the "
           "tables off any title's own disks.\n"
    ...
    _field(0x100, 1, _U8, "roster_in_use", "Roster in use", _OK,
           ...
           "**This is the character's STATUS, and the name is now wrong.** "
           "Zero means the slot is empty -- DROP CHARACTER writes it and "
           "nothing else (#104 (A converted DOS party arrives with the "
           "template save's spare characters still in it)) -- and any "
           "other value is a state the sheet "
           "puts into words. LIBRARY $38BE draws that line: `LDA $6C00 / AND "
           "#$07 / CLC / ADC #$29 / TAX / LDA #$16 / JMP $322D`, and the "
           "string table at LIBRARY $3439 (low) and $347B (high) holds ids "
           "$29-$30 as HITPOINTS, OK, GONE, DEAD, DYING, UNCONSIOUS -- the "
           "game's own spelling -- RUNNING, STONED. So the low three bits are "
           "the state and **bit 7 is a separate flag, masked off for "
           "display**: $1BF6 skips a slot whose bit 7 is set when it sums the "
           "party's strength, and the party panel draws such a name in "
           "**red**. That colour was recorded here as grey and it is not: "
           "LIBRARY $3E47 is `LDX $6E34 / CMP #$80 / BCC + / LDX #$02`, and "
           "2 is red. **CONFIRMED, and the flag is independent of the low "
           "three bits** (#235 (Two unattributed DOS byte ranges in the "
           "combat tail are dropped converting to C64, and nobody knows "
           "what they hold)): $81 -- OK with bit 7 set -- draws OK on the "
           "sheet and red in the panel, and $05 -- unconscious with bit 7 "
           "clear -- draws UNCONSIOUS on the sheet and the panel's ordinary "
           "colour, both in one boot beside three controls at $01. That is "
           "the same effect DOS gets from its own separate byte at DOS "
           "0x10D, where 0 draws the name red, so the two ports hold one "
           "flag at opposite polarities.\n"
           "**The panel is drawn from roster slot 7 down to 0**, so panel "
           "position 0 is the highest occupied slot: $3E21 is `LDA #$07 / "
           "STA $6DB4`, $3E30 calls $3189, which copies $8300 + $6DB4 * $20 "
           "into the staging page, and $3EAD decrements it. A run that reads "
           "the sheets back by panel position and calls the position a slot "
           "gets every staged value against the wrong character.\n"
           "1 OK, $82 GONE, $83 DEAD, $84 DYING, $85 UNCONSIOUS, $86 RUNNING, "
           "$87 STONED. Each is written by a routine that says what it means: "
           "POST.COM $216A sets $82 after comparing the age at 0x074 against "
           "a per-race maximum at $218E; COMBAT $0C4B turns $84 into $83 when "
           "the bleed counter $A6C0,X reaches 10, and DUNGEON $2626 chooses "
           "$85 over $83 by the same ten points below zero, which is AD&D's 0 "
           "to -9 dying and -10 dead; COMBAT $2161 walks slots 7 to 0 turning "
           "every $84 into $85 as the fight ends, which is the party binding "
           "its dying; COM.PREP $0F1F writes $86 to the side that is fleeing; "
           "SPELLE01 $AC38 writes $87 beside message 6, TURNS TO STONE.\n"
           "**Measured in the running game, not only read.** One character in "
           "PORSAVE13's Slums ambush was wounded to 1 hit point through the "
           "monitor and nothing else touched; an orc took him to 0 and the "
           "byte went 01 -> 84 on that turn, then 84 -> 85 when the party "
           "won, and 85 is what the game wrote to the save disk. Loading that "
           "disk, his sheet reads UNCONSIOUS. tools/c64/statusdrive.py.\n"
           "Four values carry an upper bit that is not bit 7 and are not "
           "explained: SPELLE04 $AA11 writes $03 beside creature type 4, "
           "undead -- Animate Dead, a DEAD thing with bit 7 clear so it still "
           "acts -- and $09 where it forces hit points to 1; SPELLE01 $ACDA "
           "writes $11 beside GETS BACK UP; SPELLE02 $AA05 writes $21. All "
           "four mask to OK or DEAD, so nothing a player sees separates them.\n"
           "The DOS enumeration is a different one at a different offset: 0 "
    ...
    _field(0x0EC, 1, _I8, "missile_attack_adjustment",
           ...
           "edition missile attack adjustment exactly: -3 at DEX 3, -2 at 4, "
           "-1 at 5, nothing from 6 to 15, +1 at 16, +2 at 17, +3 at 18, and "
           "on past 18 to +5 at 25 for a magically raised score. LIBRARY "
           "$36A8 reads it -- `LDA $6BEC` / `ADC $6C0E` -- and adds it to the "
           "roster's THAC0 when the readied weapon's type +14 has bit 1, "
           "ranged; the block below it adds the strength bonus instead when "
           "bit 2 is set. COMBAT $0899 adds it again in the attack roll.\n"
           "**It is a cache, and it goes stale**, because COM.PREP is the "
           "combat-preparation overlay: the byte is rewritten when a fight "
           "starts and at no other time anybody has found. Measured in the "
           "running game -- $7F poked into all six records of the PORSAVE13 "
           "party read back as 3, 1, 0, 0, 0, 0 four steps later in the Slums "
           "ambush, against dexterities 18, 16, 13, 12, 15, 14; and "
           "un-readying MALCYON's darts moved nothing. 96 of the 114 "
           "character records on the player's 19 save disks hold the table "
           "value for their own dexterity; all 18 that disagree hold a value "
           "*below* it, and every one belongs to a character whose dexterity "
           "wish edited upwards with no fight since. MALCYON is the clearest: "
           "1 at DEX 16 in PORSAVE2, still 1 after the edit to 18 in "
           "PORSAVE4-9, and 3 in PORSAVE11 after the next fight.\n"
           "This byte was read for a long time as spell state, on the "
           "observation that it went 0 -> 1 after combat for MALCYON and LADY "
           "KATHERINE 'and nobody else -- exactly the two spellcasters'. They "
           "are also exactly the two characters with a dexterity of 16; the "
           "other four are 12 to 15, whose table entry is zero, so there was "
           "nothing for their byte to do. #202 (Name record offset 0x0EC, "
           "which is what moves a THAC0 when darts are readied)"),
    ...
    _field(0x10D, 1, _U8, "party_order", "Party order", _MAYBE,
           ...
           "#160 (The automapper and the editor list the party backwards) "
           "showed the marching order is carried by the slot arrangement "
