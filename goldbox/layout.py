...
_DECLARED: Sequence[Field] = (
    ...
    _field(0x0E3, 5, _RAW, "region_0e3", "unknown @0x0E3", _NOPE,
           ...
           "FF in every NPC and its first two bytes are $00 in every player "
           "character, which used to be written up here as part of an "
           "eight-byte NPC marker. **There is no marker.** Every one of the "
           "five shipped MON* records a joined NPC is a copy of carries "
           "exactly 16 $FF bytes in its first 256, at the same 16 offsets in "
           "all five -- 0x0A1, 0x0A2, 0x0B7, 0x0B8, 0x0B9, 0x0BA, 0x0D1, "
           "0x0D2, 0x0D3, 0x0D4, 0x0E2, 0x0E4, 0x0E5, 0x0E6, 0x0E7, 0x0FB. "
           "The party loader overwrites six of them and leaves ten, and the "
           "'eight' were those ten minus 0x0E6-0x0E7, dropped only because a "
           "player character is non-zero there. So the set is the author's "
           "$FF fill of the monster files intersected with what a player "
           "character happens to hold zero at, and nothing in it is written "
           "or tested by the game: bit 7 of flags_0b8 is the flag the engine "
           "reads. Two of the eight turned out to be a real field in another "
           "title -- see dual_class_slot. 0x0E6-0x0E7 hold a non-zero, "
           "high-entropy per-character value in every single player "
           "character; the DOS record has one such byte in the same place, "
           "immediately before experience, which its community documentation "
           "calls MON_Index, so whatever the pair is, both ports carry it. "
           "docs/50-experiments.md and #224 (0x0B9 and 0x0BA are "
           "documented both as an NPC marker and as the dual-class slot)",
           candidate=True),
