...
DROPPED: tuple[tuple[str, str], ...] = (
    ...
    ("running_effects", "the C64 keeps a running effect in the save's own "
                        ...
                        "built. Camp-clock time is settled: "
                        "`goldbox.effects.closest_duration` turns the "
                        "source's minutes into the count and unit whose "
                        "camp-clock expiry is nearest, from the camp ageing "
                        "routine of all three titles. Two routes still age a "
                        "slot differently and are open: combat ages only unit "
                        "00, and Silver Blades' walking rule differs from "
                        "Pool's and Curse's. The value is not "
                        "-- DOS's `.SPC` data byte is not the C64 magnitude "
                        "for every id -- and neither is which slot and owner "
                        "a converted effect takes "
