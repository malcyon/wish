...
Renamed from `dos_layout.py`, and `DosShape` from `DosDeltas`
-------------------------------------------------------------
...
#: stride from `DosDeltas.item_size` rather than from here; this is the default
#: those deltas override, and slicing a `.STF` at 63 is the bug #113 closed.
ITEM_SIZE = 63
...
#: table: read `DosDeltas.race_numbers`, never this, unless the title is known.
#: It stays the module-level name so nothing that read it before #237 changed
#: meaning, and it is the default `DosDeltas` field.
RACE_NUMBERS = (
...
_DECLARED: Sequence[Field] = (
    ...
    _f(0x02E, 1, _U8, "race", "Race", _OK,
       ...
       "`DosDeltas.race_numbers`, which is per title, rather than through "
       "`RACE_NUMBERS`"),
    ...
    _f(0x0BF, 1, _U8, "combat_figure", "Combat icon slot", _OK,
       ...
       "`icon_head` and `icon_colours` do.\n"
       "**CONFIRMED out of the shipped `GAME.OVR`, not from a specimen.** "
       "Pool of Radiance's overlay holds a five-instruction loop -- "
       "`mov byte ptr es:[di+0BFh], 0` / `cmp byte ptr es:[di+0BFh], 8` / "
       "`mov al, es:[di+0BFh]` used as an index into an eight-entry array "
       "of taken-slot flags / `inc byte ptr es:[di+0BFh]` -- which is an "
       "allocation of the lowest free slot of eight. Curse's overlay holds "
       "the same loop byte for byte with the displacement changed to its "
       ...
       "calls the allocator once per character, so character *i* of a party "
       "with no NPC gets slot *i* -- the marching position, arrived at as a "
       ...
       "the destination overwrites costs nothing and the round trip needs "
       "it. `simeonpilgrim/coab` names it `icon_id` at Curse 0x143 and its "
       "`AssignPlayerIconId` (`ovr017`, `sub_4A60A`) is the loop above; "
       "`draw_combat_icon` and `chead_cbody_comspr_icon` are the readers. "
    ...
    _f(0x10C, 4, _RAW, "field_10c_10f", "status, active, side, quickfight",
       ...
       "and 1 does not, 3 of 3 against 9 of 9 across two boots -- and it is "
       "the engine's own `in_combat`, which is the same field rather than a "
       "rival reading of it (#305). Curse's Pool of Radiance importer, in "
       "its shipped `GAME.OVR` at file offset 0x01D55F, copies this byte "
       "into Curse's 0x196 in the instruction pair after the one that "
       "copies 0x10C into Curse's 0x195; `simeonpilgrim/coab` calls Curse "
       "0x195 `health_status` and 0x196 `in_combat`, and its "
       "`displayPlayerName` paints a character whose `in_combat` is false "
       "in colour 0x0C, EGA light red -- which is the red name the staging "
       "measured. 0x10E is "
...
class DosDeltas:
    ...
    The deltas: the ways this title's record departs from Pool of Radiance's
    -- never the record's own form, which is `LAYOUT` below and
...
_FORMER_NOTE = (
    ...
    "read at this title's own offsets: Curse's DEMELTINA, a human paladin 5 "
...
_PALADIN_CURES_NOTE = (
    ...
    "across four record widths and six titles: Curse's MATHEW, MARK and "
    "DEMELTINA and Gateway to the Savage Frontier's JERRICUS at the "
    "422-byte record's 0x191, Silver Blades' Guy de Valois and DEMELTINA, "
    ...
    "422-byte record keeps it in the combat tail and the 439- and 510-byte "
    "records keep it beside the class. It stays 1 after HUMAN CHANGE CLASSES: "
...
#: One `Field` object per title, because `layout_for` fills the offset in.
...
def deltas_for(what: "int | str | DosDeltas") -> DosDeltas:
    """The deltas for a record size, a key, or a `DosDeltas` already in hand.

    **The size names the deltas, and among these four it names the title
    too** -- no two of the four are the same length -- which is what lets a
    reader identify a file it was handed with no title.
    ...
    size answers "which deltas", and only the directory answers "which game".
    """
    if isinstance(what, DosDeltas):
        return what
...
def layout_for(what: "int | str | DosDeltas") -> tuple[Field, ...]:
    ...
    is where it lands.  Deltas whose widths do not add up to their own
    record size raise here rather than handing back a table that reads the
    wrong bytes -- which is the check that makes a new title cheap to try.
...
assert LAYOUTS[POOL_OF_RADIANCE.key] == LAYOUT, (
    "the Pool of Radiance deltas must reproduce the table it was read from")
