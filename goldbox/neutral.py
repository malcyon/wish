...
FIELDS: dict[str, str] = {
    ...
    "unnamed_0ab": "the identity draw the engine makes at creation and uses "
                   ...
                   "draws two at 0x0E6-0x0E7 and never rewrites them; an "
                   "Amiga Pool of Radiance record holds its own copy at the "
                   "same offset DOS does, re-cut straight across by "
                   "`goldbox.amiga_por.to_dos_record` (#378, An Amiga character "
                   "converted to DOS loses the identity byte his own "
                   "record has always held). None of the three ports ever "
                   "reads it back once written, so a writer with no source "
                   "for it is free to derive one instead, which is what a "
                   "DOS destination with no supplied value does (#216, "
                   "Every converted DOS character carries the same "
                   "identity byte at 0x0AB) -- and what it still does for a "
                   "pure DOS-to-DOS conversion even though DOS holds the "
                   "byte too, since `goldbox.dos_codec.IDENTITY_HELD_PORTS` "
                   "trusts every port but DOS's own (#378). Curse of the "
                   "Azure Bonds and Secret of the Silver Blades never draw "
