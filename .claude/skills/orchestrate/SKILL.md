...
- Past the hand-off line, once only code-reviewer and test-runner can still launch, a finding is filed on the issue and never fixed by hand.
...
Every turn resends your whole context, so a long session is the most expensive thing on this machine. You are not asked to judge your own context size: `.claude/hooks/check-context-handoff.py` reads it from the transcript and, once it passes 600k tokens, refuses every launch except code-reviewer and test-runner, and every message to a finished agent, with a message saying so; once it has refused, it keeps refusing for the rest of the session. When that refusal arrives, hand off: launch nothing new, let the agents in flight report, commit their work, review it, run the suite, push, stop the loop with ScheduleWakeup stop:true, and tell Donald to start a fresh session with /orchestrate. Do not wait for him to ask, do not retry the refused launch, and do not hand a queue item to a finished agent by message instead. The loop prompt's "never end a turn with nothing running" does not apply to a session that is winding down: ending a turn with nothing running is the point.
