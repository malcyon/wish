...
Every turn resends your whole context, so a long session is the most expensive thing on this machine. You cannot see your own context size, so you are not asked to judge it: `.claude/hooks/check-context-handoff.py` reads it from the transcript and, once it passes 300k tokens, refuses every launch except code-reviewer and test-runner, with a message saying so. When that refusal arrives, hand off: launch nothing new, let the agents in flight report, commit their work, review it, run the suite, push, commit the queue file, stop the loop with ScheduleWakeup stop:true, and tell Donald to start a fresh session with /orchestrate. Do not wait for him to ask, and do not retry the refused launch. The loop prompt's "never end a turn with nothing running" does not apply to a session that is winding down: ending a turn with nothing running is the point.
