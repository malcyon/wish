...
Read `AGENTS.md` and `INDEX.md`. Two of its rules bear directly on review:

**The standards are in `.claude/rules/`, and a subagent does not inherit them.** `CLAUDE.md` and `AGENTS.md` reach you automatically; those files do not. Read the ones that bind this work before you start: `.claude/rules/testing.md`, `.claude/rules/commits.md` and `.claude/rules/gui-text.md`. `docs/160-why-these-rules.md` carries the incidents behind them, if you need to know why a rule is there.
