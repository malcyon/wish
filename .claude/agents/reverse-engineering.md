...
brief names exists rather than sweeping the tree for anything else.

**A probe is one script, run once — not thirty `cat`s and `sed`s.** Write it,
run it, and keep it under `tools/` if it answered a question.
