...
**A bug the last release never had is not a fix to report.** Some of the
...
character editing should work but bugs are expected."* That sentence tells the
reader more than a confident one would, and it is the register Donald wants.
...
a link on the release page and stays ordinary text in the file; the written-out
...
**A line with no issue needs a second look** before you write it. Most user-
...
at `.claude/rules/documentation.md` and `.claude/rules/feature-flags.md` for
anything it does not cover itself. `docs/160-why-these-rules.md` carries the
